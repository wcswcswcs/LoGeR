...
# 论文公式与具体代码逐条对照表（LoGeR）

> 说明：下面只对论文中**与当前开源仓库可直接对应**的公式做逐条映射。对训练损失 Eq. (8)–(11)，当前仓库里**没有找到明确的训练入口/训练脚本中的直接实现**；仓库更像是以推理与评估为主，因此我会如实标注“未在当前仓库中直接暴露”。

| 论文公式 | 论文含义 | 对应代码位置 | 代码如何实现 | 我当前的理解与备注 |
|---|---|---|---|---|
| Preliminary Eq. (1)  \(W \leftarrow W - \eta \nabla_W L(f_W(k), v)\) | TTT 的通用 **update** 操作：用 key/value 对 fast weights 做在线更新 | `loger/models/ttt.py:118-165` | `fast_weight_swish_glu_weight_norm_mini_batch_apply(...)` 在 `update=True` 分支中，先用 `k,v` 计算 `w0_grad / w1_grad / w2_grad`，再做 `w_now = w_now + grad`，最后做 weight norm | **已较完整理解。** 论文把 fast weight 写成抽象的 \(W\)，代码里实际拆成三组快权重 `w0/w1/w2`；另外代码写的是 **gradient ascent** 形式，但功能上就是“基于当前 chunk 写入记忆” |
| Preliminary Eq. (2)  \(o = f_W(q)\) | TTT 的通用 **apply** 操作：用更新后的 fast weights 处理 query | `loger/models/ttt.py:167-170` | `oi = (silu(qi @ w0_now) * (qi @ w2_now)) @ w1_now` | **已较完整理解。** 论文中的 \(f_W(\cdot)\) 在代码里具体落成了 **multi-head SwiGLU fast-weight MLP** |
| Eq. (3)  \(H^{C_m} \leftarrow H^{C_m} + [\mathrm{Attn}_{frame}(\mathrm{LN}(H_i^{C_m});\theta)]\) | 每帧内部的 self-attention / spatial feature extraction | `loger/models/pi3.py` 解码主干中的基础 block 前向（TTT/SWA 插入前的原生主干逻辑）；TTT 插入位置由 `ttt_insert_after=[0,2,...,34]` 指定见 `ckpts/LoGeR/original_config.yaml` 与 `loger/models/pi3.py:224-251` | 当前仓库没有把 “Eq. (3) per-frame attention” 单独写成一个显式函数名，而是嵌在 `pi3.py` 的主解码块中；从插入位置可以反推出：TTT/SWA 是加在 frame-attention 之后、chunk/global attention 之前 | **理解基本正确，但不是 100% 可单行点名。** 我能确定“它在主干里存在且插层位置正确”，但由于作者没有把 Eq. (3) 单独封成 `Attnframe(...)` 这样的函数，代码对应是**结构级**而不是**单函数级** |
| Eq. (4)  \(H^{C_m} \leftarrow H^{C_m} + \mathrm{Attn}_{swa}([\mathrm{LN}(H^{C_{m-1}}),\mathrm{LN}(H^{C_m})];\theta)\) | 相邻 chunk 间的 SWA，对齐局部高保真上下文 | `loger/models/pi3.py:449-528` | 读取 `attn_state['history']`，把 history 与当前 chunk 拼起来，或直接走 KV cache 路径；随后 `swa_output * gate_scale + residual` 注入回主干 | **已理解主机制。** 这部分和论文“只看相邻 chunk、保留无损局部上下文”的说法是一致的 |
| Eq. (5)  \(\tilde H^{C_m} = H^{C_m} + f_{W^m}(\mathrm{LN}(H^{C_m}))\) | chunk-wise TTT 的 **apply** 步骤 | `loger/models/pi3.py:425-435` + `loger/models/ttt.py:247-317` | `pi3.py` 把 `w0/w1/w2` 和 `ttt_op_order` 打包到 `info`，调用 `self.ttt_layers[layer_idx](tokens_in, info)`，输出经 gate 后加到 `tokens_post` 上 | **已基本吃透。** 论文里写 pre-norm inside TTT；代码中 `FastWeightGluMLPMultihead` 支持 `ttt_pre_norm`，并在 `forward` 中先 `RMSNorm` 再进入 TTT |
| Eq. (6)  \(W^{m+1} = \mathcal U(W^m; H^{C_m})\) | chunk-wise TTT 的 **update** 步骤 | `loger/models/pi3.py:425-447`, `697-704`, `734-735` + `loger/models/ttt.py:118-165` | `pi3.py` 把上一 chunk 的 `w0/w1/w2` 送入 TTT；`ttt.py` 内部更新后返回新状态；`pi3.py` 再把新状态写回，供下一个 window/chunk 使用 | **已较完整理解。** 这条链路我已经能顺着代码从“状态读入 → 更新 → 状态写回”完整追踪 |
| Eq. (7)  \(H^{C_m} \leftarrow \tilde H^{C_m} + \mathrm{BiAttn}_{chunk}(\mathrm{LN}(\tilde H^{C_m});\theta)\) | chunk 内双向 attention，用于强几何推理 | `loger/models/pi3.py` 解码主干中 TTT/SWA 注入之后的原生 block 计算；相机与点图预测头接在其后，见 `loger/models/pi3.py:784-806` | 和 Eq. (3) 类似，当前仓库没有把它单独命名成 `BiAttnchunk(...)` 的显式函数；但从主干结构、插层位置和最终预测头顺序可以对上 | **理解为结构级对应。** 我能确认它在主干中承担“chunk 内双向几何推理”的位置，但不是以论文同名模块出现 |
| Eq. (8)  \(L_{local}\) | scale-invariant local pointmap loss | **当前仓库未找到直接训练实现**；最近似的是 `eval/mv_recon/criterion.py` 中的点云距离与归一化工具，如 `L21Loss`, `normalize_pointcloud_t(...)` | 没有找到论文同款 `L_local` 的训练计算入口 | **不能声称已找到。** 这一条目前只能说“论文写了，仓库未直接公开实现” |
| Eq. (9)  \(L_{pose}\) | affine-invariant relative pose loss | **当前仓库未找到直接训练实现** | 当前公开代码中没有看到论文式的 pose loss 训练逻辑 | **不能声称已找到。** |
| Eq. (10)  \(L_{global}\) | world-coordinate global pointmap loss | **当前仓库未找到直接训练实现** | 当前公开代码中没有看到论文式的 global pointmap loss 训练逻辑 | **不能声称已找到。** |
| Eq. (11)  \(L = L_{local} + L_{pose} + \lambda_{global} L_{global}\) | 总训练目标 | **当前仓库未找到直接训练实现** | 当前仓库更偏推理/评估，训练脚本与总 loss 没有在公开代码中直接暴露 | **不能声称已找到。** |
| Eq. (12)  \(\tilde T_t^{(m)} = A_m \hat T_t^{(m)}\), 其中 \(A_m = \tilde T_k^{(m-1)} (\hat T_k^{(m)})^{-1}\) | LoGeR* 的 overlap-based feedforward alignment（SE(3)） | `loger/models/pi3.py:827-845`, `1017-1132`, `1175-1240` | `se3=True` 时走 `_merge_windowed_predictions_sim3(... allow_scale=False)`；其中 `_estimate_relative_sim3(...)` 用 overlap 帧估计 `relative_rot` 和 `relative_trans`，再把该 SE(3) 变换施加到当前 window 的 poses 与 points 上 | **已较完整理解。** 名字虽然叫 `_merge_windowed_predictions_sim3`，但 `allow_scale=False` 时就退化成论文说的 **SE(3) alignment** |

## 额外说明（LoGeR）

1. **TTT 的论文公式和代码是当前两份文档里最能一一对上的部分。**
   - Eq. (1)(2)(5)(6) 和 `ttt.py + pi3.py` 的对应关系很强。

2. **Eq. (3)(4)(7) 中，SWA 的代码对应最清楚；per-frame attention / chunk-wise bidirectional attention 更像“结构级对应”。**
   - 我能确定它们在主干中的位置和顺序；
   - 但作者没有按论文变量名把这些块独立命名出来。

3. **Eq. (8)–(11) 暂时不能说“找到了对应训练代码”。**
   - 当前公开仓库里，我没有找到论文训练损失的直接实现；
   - 因此这里必须诚实标注为“未在当前仓库直接暴露”。

4. **LoGeR* 的 reset 与 Eq. (12) 的 alignment 是配套工作的。**
   - reset: `loger/models/pi3.py:654-660`, `668-669`
   - alignment: `loger/models/pi3.py:827-845`, `1017-1240`
   - 即：即便 TTT state 被 periodic reset，仍通过 overlap pose 的 feedforward alignment 保持窗口间轨迹连续。
