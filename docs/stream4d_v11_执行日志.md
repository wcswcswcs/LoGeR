...
- v11 计划的核心新方法应是 measurement bank + latent object slot / ownership posterior + core/fringe/reject export。
- 本轮没有实现完整新方法: 没有生成 `outputs/v11_measurement_bank/...`，没有完成真实 latent object slot posterior，没有执行 Phase 5 core/fringe/reject。
- 本轮实际执行的是诊断和代理路径: candidate oracle、measurement density、GT failure attribution、S2-S5 posterior proxy/control，以及 D4RT geometry protocol audit。
- B1/O1/O38/v10 regionlet/repair 等旧配置只作为 candidate pool、baseline、control 和 attribution 输入，不作为 v11 新方法结果。
- S2-S5 是 reportable method configs，但只是 posterior proxy/control，不是 v11 核心算法实现。它们没有通过 D4RT real-vs-shuffle、S0/S1、same-support gap gates，所以没有继续 tune30/final。
...
- 这是 proxy/control 实验，不是完整 v11 latent object-slot posterior。
- 目的只是检查 real D4RT maskcount proxy 是否显著强于 area/shuffle/no-track controls。
- 因未通过 gate，本轮没有继续实现 Phase 5 core/fringe/reject，也没有启动 tune30/final。
...
- 完整 v11 measurement bank + latent object slot posterior: 未实现。本轮只执行了诊断和 proxy/control，不应把旧 primitive 代理结果充作 v11 核心方法。
