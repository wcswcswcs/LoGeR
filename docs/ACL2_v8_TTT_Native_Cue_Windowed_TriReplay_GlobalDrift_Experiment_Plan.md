...
## 17. 当前执行状态与下一轮 continuation（2026-05-13）

### 17.1 是否已经探索完

结论：**原 v8 计划中的大部分旧路线已经被实际跑过，但新提出的 action space 还没有探索完。**

已经有完整 KITTI01 full run 的实验族包括：

```text
A1 WINGAM_03 repeat / global drift dashboard
B auto window trigger
C layer/head routed tri-replay
WINGAMF / C16FINE / C16ROLE / post-window / c16 long-retention / H6 oracle
```
...
它比 `WINGAM_03_repeat = 34.190276` 略好，且明显改善 `[200,300)`，但仍然卡在 `34m` 平台，距离最终目标：

```text
KITTI01 ATE < 30m
```

仍差约 `4.16m`。因此它不是成功的最终 TTT 写入策略，只能算 v8 当前最强局部 continuation。

已经确认失败或不足的方向：

```text
auto window trigger:
    AUTO_WIN_01-06 全部明显差于手工 WINGAM/C16ROLE。
    最好 AUTO_WIN_05 也只有 35.3640，说明当前 TTT-only trigger 还不能替代人工窗口。

layer/head routing:
    多数 LHROUTE 与 C16ROLE_01 完全重合或没有独立增益。
    说明已实现的 layer/head routing 没有真正打开新的 role action space。

post-window / c16 scalar refine:
    可把结果从 34.19 推到 34.158，但本质仍是 gamma/neutral/chunk16 标量细调。
    不应继续作为主线。

H6 oracle / drift-state audit:
    能解释一些 body/exit/c16/handoff drift 现象，但现有 proxy 还没有达到可直接控制 full run 的标准。
```

最关键的是：**新提出的动作空间还没有跑**：

```text
按 token update direction 相对 window drift direction 的投影，
重新划分 positive / neutral / negative replay。
```

当前代码和结果中已有 `update_conflict_energy`、layer/head risk、window drift audit，但没有把 token-level update contribution 投影到 window drift/improvement direction 后再决定 replay role。因此下一步不能继续调 `gamma / neutral / chunk16`，应进入下面的新 continuation。

### 17.2 新 action space：window-drift projected tri-replay

旧 tri-replay 的 role assignment 是风险排序：
...
这会把“高能量但有用的 continuity token”和“高能量且推动漂移的 harmful token”混在一起。新的动作空间改问：

```text
这个 token 的 TTT update direction，
相对当前 window 的 drift / drift-correction direction，
到底是在帮长期轨迹，还是在推坏长期轨迹？
```

定义每个 token/layer/head/branch 的近似 update contribution：

```text
J_i^{l,h,b} ~= token i 对 fast-weight branch b 的 pre-zeropower update direction
```

定义 window-level correction direction：

```text
D_win^{l,h,b}
```

第一版不要直接把 SE(3) drift vector 映射到 TTT 空间。更稳的做法是从已有候选之间构造 fast-weight 空间里的 correction direction：

```text
D_win = DeltaW_good - DeltaW_ref
```

例如：

```text
good = C16ROLE_01 / C16FINE_03
ref  = WINGAM_03_repeat
bad  = AUTO_WIN_* 或 WINGAM 回退候选
```

然后对 token contribution 做投影：
...
role assignment 不再只看 risk rank，而看方向：

```text
positive:
    s_i >= positive_threshold
    且 update_conflict_energy 不极高

negative:
    s_i <= negative_threshold
    或 update_conflict_energy 高且 p_i < 0
...
```text
positive token:
    写入方向与已知 drift-correction direction 同向，应长期保留。

negative token:
    写入方向与 drift-correction direction 反向，或高 conflict 且不支持 correction，应负 replay。

neutral token:
    不确定但可能承载 continuity，保持 lambda_neu 弱保留。
```

### 17.3 严格约束：no-GT proxy 先离线，不直接控制 full run

这里必须分清两种方向：

```text
oracle direction:
    可用 GT / 已有 full benchmark / C16ROLE-vs-WINGAM 差异构造。
    只能用于离线判别和解释，不能作为可发布策略。

no-GT proxy direction:
    只能来自模型内部量，例如 overlap consistency、pass1/pass2 pose delta、
    memory_state_rel_diff、update_conflict propagation、raw chunk drift proxy。
    在通过离线相关性与符号稳定性验证前，不能直接控制 full run。
```
...
```text
Phase P0:
    汇总现有 v8 结果，冻结 C16ROLE_01 为 current v8 best。

Phase P1:
    做 oracle drift-projection passive audit。
    只看 token role 是否能解释 C16ROLE_01 / C16FINE_03 为什么优于 WINGAM_03。

Phase P2:
    做 no-GT proxy drift-projection passive audit。
    只检验 proxy 与 oracle role / true window delta 的相关性。

Phase P3:
    只有 P2 通过 gate，才实现 online controller。

Phase P4:
    只有 online smoke 通过，才跑 full KITTI01。
```

### 17.4 Passive audit 设计

新增或扩展工具：

```text
tools/ttt_window_drift_projection_audit.py
```
...
--diag-dirs
    trajectory_diagnostics_c16role
    trajectory_diagnostics_h5
    trajectory_diagnostics_auto_windows
...
```text
projection_role_metrics.csv
window_direction_summary.csv
role_overlap_with_existing_tri_replay.csv
proxy_vs_oracle_projection_corr.csv
projection_audit.md
```

每个 chunk/layer/head 记录：
...
projection_p90
energy_mean
conflict_energy_mean
positive_mass_projected
neutral_mass_projected
negative_mass_projected
overlap_with_old_positive
overlap_with_old_negative
```

离线成功标准：

```text
1. C16ROLE_01 / C16FINE_03 的 projected-positive mass 高于 WINGAM_03；
2. AUTO_WIN 回退候选的 projected-negative 或 anti-correction mass 更高；
3. oracle projection role 与 [200,300) / [200,400) delta 的 Spearman >= 0.45；
4. no-GT proxy projection 与 oracle projection 的 Spearman >= 0.35；
5. no-GT proxy 在 body / exit / c16 三个窗口上的符号不能反。
```

如果 P1/P2 不过，不允许进入 full run。此时结论应是：当前 internal proxy 还不足以把 trajectory-level drift direction 映射回 token write role。

### 17.5 Online controller 候选（仅 P2 通过后启动）

新增 controller mode：

```text
TTT_WRITE_TRI_REPLAY_ROLE_SOURCE = projection
TTT_WRITE_PROJECTION_DIRECTION_SOURCE = no_gt_window_proxy
TTT_WRITE_PROJECTION_WINDOWS = body:5-9,exit:10-12,c16:16-16
```

第一批只在手工窗口上验证 action space，不同时挑战 auto-window：

| Run | Window | Role source | Gamma | Neutral | 目的 |
|---|---|---|---:|---:|---|
| `V8_PROJROLE_01` | `5-9 / 10-12` | projection sign | body `0.005`, exit `0.003` | `0.85` | 直接替换 WINGAM/C16ROLE 的 rank role |
| `V8_PROJROLE_02` | `5-9 / 10-12 / 16` | projection sign | c16 `0.0003` | `0.85` | 保留 C16ROLE 信号 |
| `V8_PROJROLE_03` | `5-9 / 10-12` | projection + conflict gate | same | `0.85` | 只有高 conflict 且反向才 negative |
| `V8_PROJROLE_04` | `5-9 / 10-12` | projection confidence fallback | same | `0.85` | 低置信 token 回退旧 WINGAM rank |
...
strong:
    ATE <= 33.80
    或 [200,300) <= 73.80 且 [400,600) 不恶化

stop:
    4 个 PROJROLE full run 都未过 C16ROLE_01，
    且 projection dashboard 没有给出新的可解释 role 分离。
```

### 17.6 如果 projection action space 仍失败

如果 projection role 也卡在 `34m` 平台，说明问题很可能不在 “token role assignment” 本身，而在更上游：

```text
1. read path / frame attention cue 对 [200,300) 的几何尺度仍不足；
2. TTT fast-weight write 只能修局部 reset-window shape，无法修全局 Sim3 / x-axis drift；
3. 当前 pose/pointmap 输出中缺少能稳定反馈长期 trajectory 的 no-GT signal。
```

那时停止 TTT write 微结构，转向新的探索计划：

```text
H9 read-write co-design:
    用 projection audit 发现的 bad windows 反向约束 read attention，而不是只改 write。

H10 reset-boundary geometry consistency:
    用 overlap pointmap / reset boundary raw pose jump 训练-free 地选择 read/write state。

H11 cross-sequence sanity:
    先跑 C16ROLE_01 / WINGAM_03 在 KITTI00/02/05，判断当前 34m 平台是否 KITTI01-specific。
```

当前下一步执行口径：

```text
不要继续调 gamma / neutral / chunk16 标量。
不要让 no-GT proxy 在未验证前控制 full run。
先做 window-drift projection passive audit，
再决定是否进入 PROJROLE full KITTI01。
```
