...
def main() -> None:
    ...
    if args.efficientsam3_checkpoint and not os.path.exists(args.efficientsam3_checkpoint):
        sys.exit(f"--efficientsam3_checkpoint not found: {args.efficientsam3_checkpoint}")
    if not args.device.startswith("cuda"):
        sys.exit("--device must be a CUDA device for EfficientSAM3 video inference")
    ...
    try:
        ...
        frontend = EfficientVideoMaskletFrontend.from_config(**build_kwargs, **frontend_kwargs)
