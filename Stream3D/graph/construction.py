...
def mask_graph_construction(args, scene_points, frame_list, dataset, flag=0, depth_max_pre=0.1):
    ...
    boundary_points, point_in_mask_matrix, mask_point_clouds, point_frame_matrix, global_frame_mask_list, depth_max_now = build_point_in_mask_matrix(args, scene_points, frame_list, dataset, depth_max_pre)
    
    # return [], [], mask_point_clouds, [], depth_max_now

    visible_frames, contained_masks, undersegment_mask_ids = process_masks(frame_list, global_frame_mask_list, point_in_mask_matrix, boundary_points, mask_point_clouds, args, flag=flag)

    if len(visible_frames) == 0:
        return [], [], mask_point_clouds, point_frame_matrix, depth_max_now
    ...
    return nodes, observer_num_thresholds, mask_point_clouds, point_frame_matrix, depth_max_now

def build_point_in_mask_matrix(args, scene_points, frame_list, dataset, depth_max_pre):
    '''
    ...
    for frame_cnt, frame_id in iterator:
        mask_dict, frame_point_cloud_ids, depth_max_now = frame_backprojection(args, dataset, scene_points, frame_id, depth_max_pre)
        # mask_dict, frame_point_cloud_ids = MC_frame_backprojection(dataset, scene_points, frame_id)
        # depth_max_now = 20
        # print(len(mask_dict), len(frame_point_cloud_ids), frame_id)
    ...
    return boundary_points, point_in_mask_matrix, mask_point_clouds, point_frame_matrix, global_frame_mask_list, depth_max_now
...
def process_one_mask(point_in_mask_matrix, boundary_points, mask_point_cloud, frame_list, global_frame_mask_list, args, flag = 0):
    '''
    ...
    for frame_id in possibly_visible_frames:
        ...
        if 1 - invisible_ratio < args.mask_visible_threshold[flag] and (np.sum(mask_id_count) - mask_id_count[0]) < 500:
            continue
        visible_num += 1
        mask_id_count[0] = 0
        max_mask_id = np.argmax(mask_id_count)
        contained_ratio = mask_id_count[max_mask_id] / np.sum(mask_id_count)
        if contained_ratio > args.contained_threshold[flag]:
    ...
    if visible_num == 0 or split_num / visible_num > args.undersegment_filter_threshold[flag]:
        return False, visible_frame, contained_mask
    else:
        return True, visible_frame, contained_mask

def process_masks(frame_list, global_frame_mask_list, point_in_mask_matrix, boundary_points, mask_point_clouds, args, flag=0):
    '''
    ...
    # print(global_frame_mask_list)
    for frame_id, mask_id in iterator:
        valid, visible_frame, contained_mask = process_one_mask(point_in_mask_matrix, boundary_points, mask_point_clouds[f'{frame_id}_{mask_id}'], frame_list, global_frame_mask_list, args, flag=flag)
