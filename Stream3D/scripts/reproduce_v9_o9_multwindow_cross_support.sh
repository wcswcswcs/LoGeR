...
cd "$STREAM3D"
mkdir -p logs outputs/audit/v9_o9_multwindow outputs/v9_multwindow data/evaluation/scannet
...
eval_own() {
  local config="$1"
  run_logged "logs/stream4d_v9_o9_${config}_eval.log" \
...
cross_eval() {
  ...
  local eval_policy="$4"
  run_logged "logs/stream4d_v9_o9_${output_config}.log" \
    "$PY" -m tools.evaluate_cross_prepoints \
      ...
      --audit-root outputs/audit/v9_o9_multwindow \
      --require-manifest \
      --allow-diagnostic-eval \
      --eval-policy "$eval_policy"
}

run_logged logs/stream4d_v9_o9_multwindow_py_compile.log \
  "$PY" -m py_compile \
...
run_logged logs/stream4d_v9_o9_g1_grid16m002_probe5_96f_stride20.log \
...
run_logged logs/stream4d_v9_o9_export.log \
...
cross_eval scannet "$O9_CONFIG" stream4d_v9_p0_on_o9_b1_multwin96_grid16_mc08_probe5 stream3d_on_d4rt_multwindow96_support
cross_eval "$O9_CONFIG" scannet stream4d_v9_o9_b1_multwin96_grid16_mc08_on_s0_probe5 method_on_stream3d_support
cross_eval "$O9_CONFIG" stream4d_32f_self_probe5 stream4d_v9_o9_b1_multwin96_grid16_mc08_on_s1_probe5 method_on_32f_support

run_logged logs/stream4d_v9_o9_multwindow_matrix_probe5.log \
  "$PY" -m tools.summarize_v9_unified_eval \
    --root . \
    --seq-list splits/scannet_v6_probe5.txt \
    --matrix-json scripts/v9_o9_multwindow_matrix_probe5.json \
    --output-prefix outputs/audit/v9_o9_multwindow/o9_multwindow_matrix_probe5 \
    --dataset scannet \
    --stream3d-config scannet

run_logged logs/stream4d_v9_o9_multwindow_reportable_scan.log \
  "$PY" -m tools.scan_reportable_configs \
    --root . \
    --configs "$O9_CONFIG" \
    --output outputs/audit/v9_o9_multwindow/reportable_config_scan_o9_probe5.md \
    --require-manifest \
    --require-eval-policy

run_logged logs/stream4d_v9_o9_multwindow_metric_integrity.log \
  "$PY" -m tools.verify_stream4d_metric_integrity \
    --orig-stream3d-root . \
    --current-root . \
    --configs "$O9_CONFIG" \
    --seq-list splits/scannet_v6_probe5.txt \
    --output outputs/audit/v9_o9_multwindow/metric_integrity_o9_probe5.md \
    --require-manifest

echo "v9 O9 multi-window cross-support done"
