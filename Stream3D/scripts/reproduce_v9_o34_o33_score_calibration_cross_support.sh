...
cd "$STREAM3D"
mkdir -p logs outputs/audit/v9_o34_o33_score_calibration outputs/v9_score_calibration data/evaluation/scannet
...
eval_own() {
  local config="$1"
  run_logged "logs/stream4d_v9_o34_${config}_eval.log" \
...
cross_eval() {
  ...
  local eval_policy="$4"
  run_logged "logs/stream4d_v9_o34_${output_config}.log" \
    "$PY" -m tools.evaluate_cross_prepoints \
      ...
      --audit-root outputs/audit/v9_o34_o33_score_calibration \
      --require-manifest \
      --allow-diagnostic-eval \
      --eval-policy "$eval_policy"
}

CONFIG="stream4d_v9_o34_o33_inside035_logarea_probe5"

run_logged logs/stream4d_v9_o34_o33_score_calibration_py_compile.log \
  "$PY" -m py_compile \
...
run_logged logs/stream4d_v9_o34_o33_inside035_logarea_rescore.log \
  "$PY" -m tools.rescore_prediction_scores \
    --root . \
    --seq-list splits/scannet_v6_probe5.txt \
    --input-config stream4d_v9_o33_o31_noexcl_refine_inside035_probe5 \
    ...
    --eval-policy own_score_calibration_o33_inside035_logarea
...
run_logged logs/stream4d_v9_o34_o33_score_calibration_matrix_probe5.log \
  "$PY" -m tools.summarize_v9_unified_eval \
    --root . \
    --seq-list splits/scannet_v6_probe5.txt \
    --matrix-json scripts/v9_o34_o33_score_calibration_matrix_probe5.json \
    --output-prefix outputs/audit/v9_o34_o33_score_calibration/o34_o33_score_calibration_matrix_probe5 \
    --dataset scannet \
    --stream3d-config scannet

run_logged logs/stream4d_v9_o34_o33_score_calibration_reportable_scan.log \
  "$PY" -m tools.scan_reportable_configs \
    --root . \
    --configs "$CONFIG" \
    --output outputs/audit/v9_o34_o33_score_calibration/reportable_config_scan_o34_probe5.md \
    --require-manifest \
    --require-eval-policy

run_logged logs/stream4d_v9_o34_o33_score_calibration_metric_integrity.log \
  "$PY" -m tools.verify_stream4d_metric_integrity \
    --orig-stream3d-root . \
    --current-root . \
    --configs "$CONFIG" \
    --seq-list splits/scannet_v6_probe5.txt \
    --output outputs/audit/v9_o34_o33_score_calibration/metric_integrity_o34_probe5.md \
    --require-manifest

echo "v9 O34 O33-score-calibration cross-support done"
