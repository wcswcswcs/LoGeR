...
class MatterportDataset:
    def __init__(self, seq_name, seg_model) -> None:
        ...
        # print(self.rgb_names)

        if seg_model == 'Cropformer':
            self.segmentation_dir = f'{self.root}/output_Cropformer/mask'
            self.object_dict_dir = f'{self.root}/output_Cropformer/object'
        if seg_model == 'SAM2':
            self.segmentation_dir = f'{self.root}/output_SAM2/mask'
            self.object_dict_dir = f'{self.root}/output_SAM2/object'
    ...
    def get_multi_segmentation(self, frame_id, multi=['_s', '_m', '_l'], align_with_depth=False):
        frame_name = self.rgb_names[frame_id][:-4]
        segmentations = []
        for x in range(len(multi)):
            segmentation_path = os.path.join(self.segmentation_dir, f'{frame_name + multi[x]}.png')
            if not os.path.exists(segmentation_path):
                assert False, f"Segmentation not found: {segmentation_path}"
            segmentation = cv2.imread(segmentation_path, cv2.IMREAD_UNCHANGED)
