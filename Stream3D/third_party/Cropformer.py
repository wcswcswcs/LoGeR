# Adopted from: https://github.com/facebookresearch/detectron2/blob/master/demo/demo.py  Copyright (c) Facebook, Inc. and its affiliates.
import argparse
import glob
...
if __name__ == "__main__":
    ...
    for i, seq_name in tqdm(enumerate(seq_name_list), total=len(seq_name_list)):
        ...
        if args.dataset == 'scannet':
            output_dir = os.path.join(seq_dir, 'output_Cropformer/mask')
        if args.dataset == 'scannetpp':
            output_dir = os.path.join('./data/scannetpp/data/', seq_name, 'output_Cropformer/mask')
        if args.dataset == 'matterport3d':
            output_dir = os.path.join(seq_dir, seq_name, 'output_Cropformer/mask')
