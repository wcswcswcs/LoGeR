#!/usr/bin/env python3
"""Run v94 Phase3C object-axis constrained cut on Phase7c unary shards."""
...
PHASE_ID = "v94_phase3C_object_axis_constrained_cut"
RUN_ID = "v94_phase3C_object_axis_constrained_cut"
OUT = ROOT / "outputs/audit/v94_phase3C_object_axis_constrained_cut"
...
def _variant_specs() -> list[dict[str, Any]]:
    return [
        {
            "variant_id": "OC0_whole_source_per_object_control",
            "family": "baseline",
            "mode": "whole_source",
            "description": "Whole-source per-object control in the object-axis constrained-cut evaluator.",
        },
        {
            "variant_id": "OC1_binary_cut_each_object",
            "family": "real",
            "mode": "constrained_cut",
            "score_scale": 12.0,
            "alpha": 0.70,
            ...
            "min_region_fraction": 0.04,
            "max_region_fraction": 0.82,
            "description": "Binary object-vs-not-object cut score followed by source-level region ownership WTA.",
        },
        {
            "variant_id": "OC2_binary_cut_with_multiobject_WTA",
            "family": "real",
            "mode": "constrained_cut",
            "score_scale": 9.0,
            "alpha": 0.78,
            ...
            "boundary_weight": 0.06,
            "min_region_fraction": 0.08,
            "max_region_fraction": 0.88,
            "description": "Softer high-recall cut with explicit source-level multi-object WTA.",
        },
        {
            "variant_id": "OC3_alpha_expansion_approx_3round",
            "family": "real",
            "mode": "constrained_cut",
            "score_scale": 8.0,
            "alpha": 0.82,
            ...
            "min_region_fraction": 0.12,
            "max_region_fraction": 0.94,
            "description": "Three-round alpha-expansion-like approximation with high recall and ownership WTA.",
        },
        {
            "variant_id": "OC4_cut_with_strong_edge_barrier",
            "family": "real",
            "mode": "constrained_cut",
            "score_scale": 12.0,
            "alpha": 0.70,
            ...
            "cut_iters": 5,
            "threshold": 0.08,
            "pairwise_weight": 0.24,
            "d4rt_weight": 0.08,
            "negative_weight": 0.20,
            "boundary_weight": 0.26,
            "min_region_fraction": 0.03,
            "max_region_fraction": 0.70,
            "description": "Strong boundary barrier cut to test whether AP50 improves when cuts stop earlier.",
        },
        {
            "variant_id": "OC5_cut_with_soft_edge_barrier",
            "family": "real",
            "mode": "constrained_cut",
            "score_scale": 10.0,
            "alpha": 0.76,
            ...
            "min_region_fraction": 0.10,
            "max_region_fraction": 0.90,
            "description": "Soft edge-barrier cut for high-recall recovery after hard barriers under-cover.",
        },
        {
            "variant_id": "OC6_cut_with_D4RT_negative_veto",
            "family": "real",
            "mode": "constrained_cut",
            "score_scale": 11.0,
            "alpha": 0.72,
            ...
            "max_region_fraction": 0.80,
            "description": "D4RT-positive / hard-negative-veto cut variant.",
        },
        {
            "variant_id": "OC_CTRL_shuffled_cut",
            "family": "control",
            "mode": "shuffled_constrained_cut",
            "score_scale": 10.0,
            "alpha": 0.76,
            "propagation_iters": 24,
            "cut_iters": 5,
            ...
            "min_region_fraction": 0.08,
            "max_region_fraction": 0.88,
            "description": "Deterministically shuffle object rows before constrained cut; must not be counted as real.",
        },
    ]
...
def _materialize_variant_rows(args: argparse.Namespace) -> dict[str, Any]:
    ...
    config_rows = [
        {
            "schema_version": "stream4d_v94_phase3C_object_axis_config_v1",
            "phase_id": PHASE_ID,
            "run_id": RUN_ID,
        ...
        with np.load(shard_path, allow_pickle=False) as data:
            ...
            for source_idx, raw_source_key in enumerate(source_keys):
                ...
                if not meta or not node_map:
                    failure_rows.append(
                        {
                            "schema_version": "stream4d_v94_phase3C_object_axis_failure_v1",
                            "phase_id": PHASE_ID,
                            "run_id": RUN_ID,
                ...
                for spec in specs:
                    ...
                    cut_selections = _select_cut_regions_for_variant(spec, object_pairs, scores, prob, edge_u, edge_v, edge_w, nodes_typed, device)
                    for object_local_idx, object_key in object_pairs:
                        selected, cut_scores, cut_diag = cut_selections.get(int(object_local_idx), (set(), np.zeros(r_regions, dtype=np.float32), {}))
                        if not selected:
                            continue
                        ...
                        selected_cut_score = [float(cut_scores[idx]) for idx in selected]
                        selected_unary = [float(scores[int(object_local_idx), idx]) for idx in selected]
                        area = int(np.count_nonzero(mask))
                        area_ratio = float(area / max(1, source_area))
                        object_score = float(0.50 * _mean(selected_cut_score) + 0.25 * _mean(selected_prob) + 0.15 * _mean(selected_unary) + 0.10 * min(1.0, area_ratio))
                        score_protocol = "object_axis_gpu_constrained_cut_score_prob_unary_area"
                        score_protocol_counts[score_protocol] += 1
                        generated_row = {
                            "schema_version": "stream4d_v94_phase3C_object_axis_generated_mask_v1",
                            "phase_id": PHASE_ID,
                            "run_id": RUN_ID,
                            ...
                            "mean_selected_cut_score": _mean(selected_cut_score),
                            "cut_iteration_count": cut_diag.get("cut_iteration_count", ""),
                            "energy_before": cut_diag.get("energy_before", ""),
                            "energy_after": cut_diag.get("energy_after", ""),
                            "energy_delta": cut_diag.get("energy_delta", ""),
                            "label_changed_region_count": cut_diag.get("label_changed_region_count", ""),
                            "object_area_change_ratio": cut_diag.get("object_area_change_ratio", ""),
                            "region_ownership_conflict_count": cut_diag.get("region_ownership_conflict_count", ""),
                            "pre_wta_region_ownership_conflict_count": cut_diag.get("pre_wta_region_ownership_conflict_count", ""),
                            "cannot_link_violation_count": cut_diag.get("cannot_link_violation_count", ""),
                            "component_count_proxy": cut_diag.get("component_count_proxy", ""),
                            "positive_seed_fraction": cut_diag.get("positive_seed_fraction", ""),
                            "edge_source": edge_source,
                            "edge_count_used": edge_count,
                            "score_protocol": score_protocol,
                            "solver_backend": "torch_cuda_edge_propagation_plus_constrained_cut" if str(device).startswith("cuda") else "torch_cpu_edge_propagation_plus_constrained_cut",
                        ...
                        mv_row = {
                            ...
                            "materializable": True,
                            "selection_reason": f"v94_phase3C_object_axis_{spec['mode']}_{score_protocol}",
                        }
                        writer.add(variant_id, mask, object_score, generated_row, mv_row)
                        materialized_object_count += 1
                        assignment_rows.append(
                            {
                                "schema_version": "stream4d_v94_phase3C_object_axis_assignment_summary_v1",
                                "phase_id": PHASE_ID,
                                "run_id": RUN_ID,
                                "variant_id": variant_id,
                                "scene_id": scene,
                ...
                source_summary_rows.append(
                    {
                        "schema_version": "stream4d_v94_phase3C_object_axis_source_summary_v1",
                        "phase_id": PHASE_ID,
                        "run_id": RUN_ID,
                        "source_key": raw_source_key,
    ...
    for row in aggregate_rows:
        ...
        variant_metric_rows.append(
            {
                "schema_version": "stream4d_v94_phase3C_object_axis_variant_metric_v1",
                "phase_id": PHASE_ID,
                "run_id": RUN_ID,
                "variant_id": variant_id,
                "scene_id": "ALL_DEV",
                ...
                "mean_selected_cut_score": _mean([_num(item.get("mean_selected_cut_score")) for item in group]),
                "cut_iteration_count_mean": _mean([_num(item.get("cut_iteration_count")) for item in group]),
                "energy_before_mean": _mean([_num(item.get("energy_before")) for item in group]),
                "energy_after_mean": _mean([_num(item.get("energy_after")) for item in group]),
                "energy_delta_mean": _mean([_num(item.get("energy_delta")) for item in group]),
                "label_changed_region_count_mean": _mean([_num(item.get("label_changed_region_count")) for item in group]),
                "object_area_change_ratio_mean": _mean([_num(item.get("object_area_change_ratio")) for item in group]),
                ...
                "positive_seed_fraction_mean": _mean([_num(item.get("positive_seed_fraction")) for item in group]),
            }
        )
    ...
    for row in variant_metric_rows:
        ...
        gate_rows.append(
            {
                "schema_version": "stream4d_v94_phase3C_object_axis_gate_v1",
                "phase_id": PHASE_ID,
                "run_id": RUN_ID,
                "variant_id": variant_id,
                "family": spec.get("family", ""),
                "phase3C_candidate_gate_pass": candidate_gate,
            ...
            failure_rows_out.append(
                {
                    ...
                    "run_id": RUN_ID,
                    "variant_id": variant_id,
                    "failure_type": "OBJECT_AXIS_CONSTRAINED_CUT_DEV_GATE_FAILED",
                    "MV_AP_window": mv_ap,
                    "MV_AP50_window": mv_ap50,
                    "phase3C_candidate_gate_pass": candidate_gate,
                    "repair_direction": "If constrained cut under-covers, soften boundary/background terms; if it over-fragments or stays below A/B, enter object-axis component pooling or mark object-axis field signal insufficient.",
                    "uses_gt_for_prediction": False,
                    "uses_future": False,
                }
            )
    ...
    summary = {
        "schema": "stream4d_v94_phase3C_object_axis_constrained_cut_summary_v1",
        "phase_id": PHASE_ID,
        "run_id": RUN_ID,
        "decision": "PASS_V94_PHASE3C_OBJECT_AXIS_CONSTRAINED_CUT_GATE" if best_real_gate else "NO_GO_V94_PHASE3C_OBJECT_AXIS_CONSTRAINED_CUT_GATE",
        "created_at": created_at,
        ...
        "phase3C_candidate_gate_pass": best_real_candidate_gate,
        "previous_object_axis_best_MV_AP_window": previous_object_axis_ap,
        "previous_object_axis_best_MV_AP50_window": previous_object_axis_ap50,
