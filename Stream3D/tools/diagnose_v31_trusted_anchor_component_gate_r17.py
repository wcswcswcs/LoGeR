...
from tools.diagnose_v31_e8_graph_split_r13 import _positive_pair_counts, _split_ids
...
def _component_sets(
    ...
    *,
    ...
    comps, _stats = _split_ids(
        list(_row_core_ids(row)),
        positive_counts,
        negative_counts,
        pos_min_count=int(pos_min_count),
        neg_block_count=int(neg_block_count),
        min_component_size=int(min_component_size),
        keep_unlinked_as_component=False,
    )
    return [set(int(v) for v in comp) for comp in comps]


def _build_component_gated_support(
    *,
    ...
    positive_counts: Counter[tuple[int, int]],
    negative_counts: Counter[tuple[int, int]],
    component_cache: dict[tuple[str, int, int, int], list[set[int]]],
    pos_min_count: int,
    neg_block_count: int,
    min_component_size: int,
    evidence_topn: int,
    ...
    for evidence_idx, row in enumerate(evidence):
        ...
        row_key = str(row.get("seed_id") or row.get("proposal_id") or evidence_idx)
        cache_key = (row_key, int(pos_min_count), int(neg_block_count), int(min_component_size))
        comps = component_cache.get(cache_key)
        if comps is None:
            comps = _component_sets(
                row,
                positive_counts,
                negative_counts,
                pos_min_count=int(pos_min_count),
        ...
        selected_components = [comp for comp in comps if comp & seed_ids]
        if not selected_components:
            blocked_tubes += len(core)
            continue
        add_ids = set().union(*selected_components)
        matched_rows += 1
        matched_components += len(selected_components)
    ...
    stats: dict[str, Any] = {
        ...
        "evidence_row_matched_count": int(matched_rows),
        "component_matched_count": int(matched_components),
        "component_gated_added_tube_count": int(added_tubes),
        "component_gated_blocked_tube_count": int(blocked_tubes),
...
def _scene_result(
    *,
    ...
    support, support_stats = _build_component_gated_support(
        role_rows=role_rows,
        ...
        pos_min_count=int(config["pos_min_count"]),
        neg_block_count=int(config["neg_block_count"]),
    ...
    row = {
        ...
        **{f"r17_{key}": value for key, value in {**seed_stats, **support_stats, **assign_stats}.items()},
...
def _search_configs() -> list[dict[str, Any]]:
    ...
    for idx, (
        ...
        configs.append(
            {
                "solver": f"R17_component_gate_search_{idx:04d}",
                ...
                "temporal_gain": 0.30,
                "risk_gain": 0.35,
                "negative_min_count": int(neg_block_count),
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    negative_counts_by_scene = {}
    negative_adjacency_by_scene = {}
    component_cache_by_scene: dict[str, dict[tuple[str, int, int, int], list[set[int]]]] = {}
    for scene in scenes:
        print(f"[r17] building pair counts {scene}", flush=True)
        ...
        positive_counts_by_scene[scene] = pos_counts
        negative_counts_by_scene[scene] = neg_counts
        negative_adjacency_by_scene[scene] = _adjacency(neg_counts)
        component_cache_by_scene[scene] = {}
        print(
            f"[r17] {scene} measurements={diag.get('measurement_count')} positive_pairs={len(pos_counts)} negative_pairs={len(neg_counts)}",
            flush=True,
        )
    ...
    for cfg_idx, config in enumerate(configs):
        print(f"[r17] config {cfg_idx + 1}/{len(configs)} {config['solver']}", flush=True)
        for control_kind in requested_controls:
            ...
            for scene in scenes:
                ...
                row, selected = _scene_result(
                    ...
                    config=config,
                    positive_counts=positive_counts_by_scene[scene],
                    negative_counts=negative_counts_by_scene[scene],
                    component_cache=component_cache_by_scene[scene],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Diagnose v31 r17 positive-component anchored coverage assignment.")
    parser.add_argument("--controls", default="real")
    parser.add_argument("--out-dir", default="outputs/audit/v31_slot_ownership")
    parser.add_argument("--summary-name", default="r17_component_gate_assignment_real_summary.csv")
