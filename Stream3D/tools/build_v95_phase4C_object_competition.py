#!/usr/bin/env python3
"""Run v95 Phase4 family C multi-label object competition."""
...
from tools import build_v95_phase4_core_conditioned_expansion as base
from tools import run_v90_dev_extent_score_cross_audit as phase7d
from tools import run_v91_phase4_radius_sweep as radius_sweep


PHASE_ID = "v95_phase4_object_competition"
RUN_ID = "v95_phase4_familyC_object_competition"
OUT = ROOT / "outputs/audit/v95_phase4_familyC_object_competition"
...
def _variant_specs() -> list[dict[str, Any]]:
    return [
        {
            "variant_id": "C0_argmax_query_only",
            "family": "real",
            "margin_floor": -999.0,
            "risk_cap": 1.0,
            "area_cap": 0.72,
            "fallback_fraction": 0.0,
            "description": "Assign every region to the highest object-unary query owner.",
        },
        {
            "variant_id": "C1_argmax_unknown_background",
            "family": "real",
            "margin_floor": 0.04,
            "risk_cap": 0.55,
            "area_cap": 0.66,
            "fallback_fraction": 0.0,
            "description": "Argmax competition with low-margin or high-risk regions retained as unknown/background.",
        },
        {
            "variant_id": "C2_argmax_source_preserve_fallback",
            "family": "real",
            "margin_floor": 0.00,
            "risk_cap": 0.70,
            "area_cap": 0.82,
            "fallback_fraction": 0.22,
            "description": "Argmax competition plus source-contained top-unary fallback for undercovered objects.",
        },
        {
            "variant_id": "C3_high_precision_competition",
            "family": "real",
            "margin_floor": 0.10,
            "risk_cap": 0.35,
            "area_cap": 0.52,
            "fallback_fraction": 0.0,
            "description": "High-precision competition with strict margin/risk gating.",
        },
        {
            "variant_id": "C4_uncertain_source_preserve",
            "family": "real",
            "margin_floor": 0.02,
            "risk_cap": 0.75,
            "area_cap": 0.90,
            "fallback_fraction": 0.35,
            "description": "Higher-recall competition that preserves uncertain source-local high-unary support.",
        },
    ]


def _iter_source_groups(path: Path, max_sources: int = 0):
    current_source: tuple[str, str, int, int] | None = None
    current_objects: dict[str, list[dict[str, str]]] = defaultdict(list)
    source_count = 0
    with path.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            source = (row["scene_id"], row["window_id"], base._int(row["frame_id"]), base._int(row["source_mask_id"]))
            if current_source is not None and source != current_source:
                yield current_source, dict(current_objects)
                source_count += 1
                if max_sources > 0 and source_count >= max_sources:
                    return
                current_objects = defaultdict(list)
            current_source = source
            current_objects[str(row["object_id"])].append(dict(row))
    if current_source is not None and (max_sources <= 0 or source_count < max_sources):
        yield current_source, dict(current_objects)


def _allowed_objects_by_query_support(query_root: Path, min_support: int) -> set[str] | None:
    if min_support <= 1:
        return None
    counts: dict[str, int] = defaultdict(int)
    with (query_root / "object_query_rows.csv").open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
    ...
    return {object_id for object_id, count in counts.items() if count >= min_support}


def _prepare_object_rows(
    object_rows: dict[str, list[dict[str, str]]],
    nodes: dict[int, dict[str, str]],
    source_area: float,
    allowed_objects: set[str] | None,
) -> dict[str, list[dict[str, Any]]]:
    out: dict[str, list[dict[str, Any]]] = {}
    for object_id, rows in object_rows.items():
        if allowed_objects is not None and object_id not in allowed_objects:
            continue
        fixed: list[dict[str, Any]] = []
        for row in rows:
            idx = base._int(row.get("region_index"))
            node = nodes.get(idx)
            if not node:
                continue
            area = max(1.0, base._num(node.get("pixel_count"), base._num(node.get("area_px"), 1.0)))
            item = dict(row)
            item["region_area_ratio"] = float(area / max(1.0, source_area))
            fixed.append(item)
        if fixed:
            out[object_id] = fixed
    return out
...
def _competition_select(object_rows: dict[str, list[dict[str, Any]]], spec: dict[str, Any]) -> dict[str, set[int]]:
    by_region: dict[int, list[tuple[str, dict[str, Any]]]] = defaultdict(list)
    selected: dict[str, set[int]] = {object_id: set() for object_id in object_rows}
    for object_id, rows in object_rows.items():
        for row in rows:
            idx = int(row["region_index"])
            by_region[idx].append((object_id, row))
            if base._bool(row.get("is_core_region")):
                selected[object_id].add(idx)
    margin_floor = float(spec.get("margin_floor", 0.0))
    risk_cap = float(spec.get("risk_cap", 1.0))
    unknown_count = 0
    for idx, candidates in by_region.items():
        ranked = sorted(candidates, key=lambda item: base._num(item[1].get("object_unary_score")), reverse=True)
        if not ranked:
            continue
        best_object, best_row = ranked[0]
        best_score = base._num(best_row.get("object_unary_score"))
        second = base._num(ranked[1][1].get("object_unary_score")) if len(ranked) > 1 else -999.0
        margin = best_score - second
        if margin < margin_floor or base._num(best_row.get("risk_score")) > risk_cap:
            unknown_count += 1
            continue
        selected[best_object].add(idx)
    fallback_fraction = float(spec.get("fallback_fraction", 0.0))
    if fallback_fraction > 0:
        for object_id, rows in object_rows.items():
            target_n = max(len(selected[object_id]), int(math.ceil(fallback_fraction * len(rows))))
            for row in sorted(rows, key=lambda item: base._num(item.get("object_unary_score")), reverse=True)[:target_n]:
                if base._num(row.get("risk_score")) <= risk_cap:
                    selected[object_id].add(int(row["region_index"]))
    return {object_id: base._cap_by_area(object_rows[object_id], indices, float(spec.get("area_cap", 1.0))) for object_id, indices in selected.items()}
...
def _object_score(rows: list[dict[str, Any]], selected: set[int]) -> float:
    ...
    return float(
        np.clip(
            0.50
            + 0.42 * base._mean([base._num(row.get("object_unary_score")) for row in selected_rows])
            + 0.10 * base._mean([base._num(row.get("proto_similarity")) for row in selected_rows])
...
def _gate_rows(
    ...
    for row in variant_metric_rows:
        ...
        gate_rows.append(
            {
                "schema_version": "stream4d_v95_phase4C_variant_gate_v1",
                "phase_id": PHASE_ID,
                "run_id": RUN_ID,
        ...
        if spec.get("family") == "real" and not pass_gate:
            failure_rows.append(
                {
                    "schema_version": "stream4d_v95_phase4C_failure_v1",
                    "phase_id": PHASE_ID,
                    "run_id": RUN_ID,
                    "variant_id": variant_id,
                    "failure_type": "PHASE4_FAMILY_C_NO_CANDIDATE_GATE",
                    "MV_AP_window": mv_ap,
                    "MV_AP50_window": mv_ap50,
                    "repair_direction": (
                        "Family C object competition unary is not object-specific enough; "
                        "return to Phase3 query quality or object candidate stability per v95 plan."
                    ),
                    "uses_gt_for_prediction": False,
                    "uses_future": False,
                }
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if phase3.get("decision") != "PASS_V95_PHASE3_OBJECT_QUERY_READY":
        raise RuntimeError("v95 Phase3 must pass before Phase4C")
    specs = _variant_specs()
    allowed_objects = _allowed_objects_by_query_support(query_root, int(args.min_query_support))
    ...
    if int(args.max_sources) > 0:
        allowed_sources = set()
        for source, _objects in _iter_source_groups(query_root / "region_object_unary_rows.csv", int(args.max_sources)):
    ...
    config_rows = [
        {
            "schema_version": "stream4d_v95_phase4C_variant_config_v1",
            "phase_id": PHASE_ID,
            "run_id": RUN_ID,
    ...
    for source_key, object_raw_rows in _iter_source_groups(query_root / "region_object_unary_rows.csv", int(args.max_sources)):
        ...
        object_rows = _prepare_object_rows(object_raw_rows, nodes, source_area, allowed_objects)
        if not object_rows:
            continue
        processed_sources += 1
        for spec in specs:
            variant_id = spec["variant_id"]
            selected_by_object = _competition_select(object_rows, spec)
            for object_id, selected in selected_by_object.items():
                ...
                generated_rows.append(
                    {
                        "schema_version": "stream4d_v95_phase4C_generated_mask_v1",
                        "phase_id": PHASE_ID,
                        "run_id": RUN_ID,
                        "variant_id": variant_id,
                        "scene_id": scene,
                ...
                mv_rows.append(
                    {
                        ...
                        "materializable": True,
                        "selection_reason": f"v95_phase4C_competition_from_{phase3.get('selected_query_family', '')}",
                    }
                )
                ownership_rows.append(
                    {
                        "schema_version": "stream4d_v95_phase4C_ownership_audit_v1",
                        "phase_id": PHASE_ID,
                        "run_id": RUN_ID,
                        "variant_id": variant_id,
                        "scene_id": scene,
    ...
    for spec in specs:
        ...
        variant_metric_rows.append(
            {
                "schema_version": "stream4d_v95_phase4C_variant_metric_v1",
                "phase_id": PHASE_ID,
                "run_id": RUN_ID,
                "variant_id": variant_id,
                "family": spec["family"],
    ...
    summary = {
        "schema": "stream4d_v95_phase4C_object_competition_summary_v1",
        "phase_id": PHASE_ID,
        "run_id": RUN_ID,
        "created_at": base._created_at(),
        "decision": "PASS_V95_PHASE4_FAMILY_C_CANDIDATE_GATE" if any_pass else ("SMOKE_V95_PHASE4C_MATERIALIZATION_ONLY" if bool(args.skip_eval) else "NO_GO_V95_PHASE4_FAMILY_C_NO_CANDIDATE_GATE"),
        "family": "C_multi_label_object_competition",
        "query_root": base._rel(query_root),
        "selected_query_family": phase3.get("selected_query_family", ""),
        "min_query_support": int(args.min_query_support),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-query-support", type=int, default=1)
