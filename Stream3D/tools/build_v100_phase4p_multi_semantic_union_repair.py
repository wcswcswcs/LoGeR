#!/usr/bin/env python3
"""Phase4p GT-free multi-semantic history union repair."""
...
OUT_DIR = AUDIT_ROOT / "v100_phase4p_multi_semantic_union_repair"
...
def _retag(row: dict[str, Any]) -> dict[str, Any]:
    new = dict(row)
    new["phase_id"] = "v100_phase4p_multi_semantic_union_repair"
    return new


def _source_edges() -> dict[str, list[dict[str, Any]]]:
    hist_path = PHASE4K_DIR / "history_object_rows.parquet"
    out: dict[str, list[dict[str, Any]]] = {}
    for variant_id in [
        "HMS1_sem_tau0p78_margin0p04_age4",
        "HMS2_sem_tau0p72_margin0p06_age8",
        "HMS3_sem_tau0p66_margin0p08_age99",
        "HMS4_sem_tau0p60_margin0p12_age99",
    ]:
        key = variant_id.split("_")[0].lower()
        out[key] = [
            _retag(row)
            for row in p4o._semantic_history_edges(
                hist_path,
                source_id=f"phase4k_{key}",
    ...
    out["phase4h_best"] = [
        _retag(row)
        for row in p4o._merge_csv_edges(
...
def _apply_edges(ids: list[str], edges: list[dict[str, Any]], *, variant_id: str) -> tuple[dict[str, str], list[dict[str, Any]]]:
    ...
    for row in edges:
        ...
        if dsu.union(a, b):
            new = dict(row)
            new["schema_version"] = "stream4d_v100_phase4p_union_edge_row_v1"
            new["phase_id"] = "v100_phase4p_multi_semantic_union_repair"
            new["variant_id"] = variant_id
            new["union_policy"] = "dsu_union_multiple_gt_free_semantic_history_variants"
            accepted.append(new)
...
def _apply_mapping(rows: list[dict[str, Any]], mapping: dict[str, str], *, variant_id: str) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        new = dict(row)
        new["schema_version"] = "stream4d_v100_phase4p_scene_mv_object_frame_mask_row_v1"
        new["phase_id"] = "v100_phase4p_multi_semantic_union_repair"
        new["variant_id"] = variant_id
        ...
        new["object_id_policy"] = "union_multi_semantic_history_evidence_identity"
        new["history_memory_scope"] = "phase4k_multi_semantic_variants_plus_optional_phase4h_best_no_gt"
...
def _artifact_rows(paths: list[tuple[Path, str, str]]) -> list[dict[str, Any]]:
    return [
        {
            "schema_version": "stream4d_v100_phase4p_artifact_manifest_row_v1",
            "phase_id": "v100_phase4p_multi_semantic_union_repair",
...
def main() -> int:
    ...
    if not bool(phase2c.get("phase2c_pass")):
        raise RuntimeError("Phase4p requires v100 Phase2c overlap3 local pass")
    ...
    edge_groups = _source_edges()

    variant_specs = [
        {
            "variant_id": "HMX0_hms2_plus_phase4h_best_reference",
            "families": ["hms2", "phase4h_best"],
            "notes": "Reference equivalent to Phase4o HCU3 after replay repair.",
        },
        {
            "variant_id": "HMX1_hms1_hms2_union",
            "families": ["hms1", "hms2"],
            "notes": "Union of the two best holdout semantic memories.",
        },
        {
            "variant_id": "HMX2_hms1_hms2_hms3_union",
            "families": ["hms1", "hms2", "hms3"],
            "notes": "Union stale-tolerant semantic memory with HMS1/HMS2.",
        },
        {
            "variant_id": "HMX3_all_hms_semantic_union",
            "families": ["hms1", "hms2", "hms3", "hms4"],
            "notes": "Union all Phase4k semantic scene-repair variants.",
        },
        {
            "variant_id": "HMX4_all_hms_plus_phase4h_best_union",
            "families": ["hms1", "hms2", "hms3", "hms4", "phase4h_best"],
            "notes": "Maximal GT-free semantic/evidence union in this family.",
        },
    ]
    ...
    for spec in variant_specs:
        ...
        config_rows.append(
            {
                "schema_version": "stream4d_v100_phase4p_variant_config_row_v1",
                "phase_id": "v100_phase4p_multi_semantic_union_repair",
        ...
        for split in ["dev", "holdout"]:
            ...
            row = dict(scene_agg)
            row["schema_version"] = "stream4d_v100_phase4p_metric_aggregate_row_v1"
            row["phase_id"] = "v100_phase4p_multi_semantic_union_repair"
            ...
            row["metric_composition"] = "local_window_from_phase2c_chunk_ids_scene_from_phase4p_union_ids"
            ...
            for name, source_id in source_id_by_family.items():
                row[f"{name}_source_edge_count"] = sum(1 for edge in split_edges if edge["source_id"] == source_id)
            ...
            for item in per_scene:
                item["phase_id"] = "v100_phase4p_multi_semantic_union_repair"
    ...
    failure_rows = [
        {
            "schema_version": "stream4d_v100_phase4p_failure_row_v1",
            "phase_id": "v100_phase4p_multi_semantic_union_repair",
            "failure_id": row["gate_id"],
            "severity": row["severity"],
            "expected": row["expected"],
            "observed": row["observed"],
            "repair_direction": "If multi-semantic union does not clear scene gates, existing semantic variants do not contain enough complementary GT-free identity evidence.",
        }
    ...
    _write_csv(
        ...
        [
            {
                "schema_version": "stream4d_v100_phase4p_performance_row_v1",
                "phase_id": "v100_phase4p_multi_semantic_union_repair",
                "case_id": "multi_semantic_union_and_v65_eval",
    ...
    _write_csv(
        ...
        _artifact_rows(
            [
                ...
                (gate_csv, "csv", "phase4p gates"),
                (failure_csv, "csv", "phase4p failures if any"),
                (performance_csv, "csv", "runtime and row counts"),
            ]
        ),
    )
    summary = {
        "schema_version": "stream4d_v100_phase4p_multi_semantic_union_repair_summary_v1",
        "phase_id": "v100_phase4p_multi_semantic_union_repair",
        "created_at": datetime.now().isoformat(timespec="seconds"),
        "runtime_sec": time.time() - started,
        "decision": "PASS_ENTER_PHASE5" if phase4p_pass else "BLOCK_PHASE5_REPAIR_MULTI_SEMANTIC_UNION",
        "phase4p_pass": phase4p_pass,
        ...
        "uses_gt_for_prediction": False,
        "metric_composition": "local_window_from_phase2c_chunk_ids_scene_from_phase4p_union_ids",
        "outputs": {
    ...
    return 0 if phase4p_pass else 2
