#!/usr/bin/env python3
"""Summarize ACL2 v118 Stage4-R23 LingBot anchor-initialization repair."""
...
MANIFEST = STAGE / "summary/stage4_r23_lingbot_ai_anchor_initialization_manifest.json"
...
def summarize_rows(manifest: dict) -> list[dict]:
    ...
    for method, meta in methods.items():
        for seq in ("00", "02"):
            ...
            row = {
                "schema": "acl2_v118tf_stage4_r23_lingbot_ai_anchor_initialization_row_v1",
                "dataset": DATASET,
                "seq": seq,
                "method": method,
                "branch": "LB-AI",
...
def main() -> None:
    ...
    if not complete:
        decision = "AI_ANCHOR_INIT_RUNTIME_INCOMPLETE"
    elif not action_fidelity:
        decision = "AI_ANCHOR_INIT_ACTION_FIDELITY_NO_GO"
    elif candidate_better_all_controls and baseline_gate:
        decision = "AI_ANCHOR_INIT_PROMOTION_CANDIDATE_FOUND"
    else:
        decision = "AI_ANCHOR_INIT_CONTROL_OR_BASELINE_NO_GO"

    out = STAGE / "summary"
    summary = {
        "schema": "acl2_v118tf_stage4_r23_lingbot_ai_anchor_initialization_summary_v1",
        "stage4_r23_decision": decision,
        ...
        "boundary": (
            "R23 tests LB-AI anchor initialization by replacing the default first8 scale frames with "
            "same-count first32 semantic/random/reverse anchor selections. It does not alter trajectory read/eviction policy."
        ),
        "outputs": {
            "rows": str((out / "stage4_r23_lingbot_ai_anchor_initialization_rows.csv").relative_to(ROOT)),
            "summary": str((out / "stage4_r23_lingbot_ai_anchor_initialization_summary.json").relative_to(ROOT)),
            "report": str((out / "STAGE4_R23_LINGBOT_AI_ANCHOR_INITIALIZATION_REPORT.md").relative_to(ROOT)),
        },
    }
    out.mkdir(parents=True, exist_ok=True)
    write_csv(out / "stage4_r23_lingbot_ai_anchor_initialization_rows.csv", rows)
    (out / "stage4_r23_lingbot_ai_anchor_initialization_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    report = [
        "# Stage4-R23 LingBot AI Anchor Initialization Summary",
        "",
        f"decision: `{decision}`",
    ...
    (out / "STAGE4_R23_LINGBOT_AI_ANCHOR_INITIALIZATION_REPORT.md").write_text(
        "\n".join(report) + "\n",
        encoding="utf-8",
    )
