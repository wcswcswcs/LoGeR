#!/usr/bin/env python3
"""Phase 5 lightweight washout attribution for v26 h10/h15 diagnostic rows.
...
def _read_metric_row(path: Path, candidate: str) -> Dict[str, str]:
    if not path.exists():
        return {}
...
def main() -> None:
    ...
    h10_metric = _read_metric_row(args.h10_report_csv, args.candidate)
    h15_metric = _read_metric_row(args.h15_report_csv, args.candidate)
    ...
    (args.out_dir / "phase5_washout_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )

    md = [
        "# v26 Phase 5 Washout Attribution",
        "",
    ...
    md.extend([
        ...
        "",
        "The h15 rollout adds extra TTT updates after the h10 endpoint, while SWA source replacement remains zero in the landed hook summaries.",
        "This supports TTT/scale-state tail update as the most plausible washout path in the available evidence, but not as a full tensor-state proof.",
        "",
    ])
