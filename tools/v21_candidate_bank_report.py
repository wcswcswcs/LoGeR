#!/usr/bin/env python3
"""Aggregate ACL2 v21 short-rollout metrics and durability gates."""
...
def _postprocess_outputs(out_dir: Path) -> None:
    ...
    md_new = out_dir / "acl2_v21_candidate_bank_report.md"
    if md_old.exists():
        text = md_old.read_text(encoding="utf-8")
        text = text.replace(
            "# ACL2 v18 True Action Candidate Report",
            "# ACL2 v21 ContextSkip / SemanticAllMemory Candidate Report",
        )
        text = text.replace("v18 true-action", "v21 short-rollout")
    ...
    if summary_path.exists():
        ...
        summary = [{}]
    if summary:
        summary[0]["phase"] = "v21 contextskip semantic-all-memory persistence candidate bank"
