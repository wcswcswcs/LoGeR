#!/usr/bin/env python3
"""Build and summarize R77 source-frame refinements for seq08 exclude-6/7."""
...
STAGE = RESULT_ROOT / "stage4_r77_lingbot_ar_fresh_exclude67_source_refinement_08"
...
R74_EXCLUDE67 = "lingbot_map_stream_sdpa_v118_r74_fresh_risk_only_exclude_6_7_seq08"
...
RELIABILITY_0_5 = {
    0: 1.0,
    1: 0.7440474189674506,
    2: 0.49622028751162905,
    3: 0.5075810293471698,
    4: 0.33713373322071416,
    5: 0.09308147816653231,
}

METHOD_SPECS = [
    {
        "role": "exclude67_leaveout_0",
        "method": "lingbot_map_stream_sdpa_v118_r77_fresh_exclude67_leaveout_0_seq08",
        "policy": "R77_FRESH_EXCLUDE67_LEAVEOUT_0",
        "source_weights": {1: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0},
    },
    {
        "role": "exclude67_leaveout_1",
        "method": "lingbot_map_stream_sdpa_v118_r77_fresh_exclude67_leaveout_1_seq08",
        "policy": "R77_FRESH_EXCLUDE67_LEAVEOUT_1",
        "source_weights": {0: 1.0, 2: 1.0, 3: 1.0, 4: 1.0, 5: 1.0},
    },
    {
        "role": "exclude67_leaveout_2",
        "method": "lingbot_map_stream_sdpa_v118_r77_fresh_exclude67_leaveout_2_seq08",
        "policy": "R77_FRESH_EXCLUDE67_LEAVEOUT_2",
        "source_weights": {0: 1.0, 1: 1.0, 3: 1.0, 4: 1.0, 5: 1.0},
    },
    {
        "role": "exclude67_leaveout_3",
        "method": "lingbot_map_stream_sdpa_v118_r77_fresh_exclude67_leaveout_3_seq08",
        "policy": "R77_FRESH_EXCLUDE67_LEAVEOUT_3",
        "source_weights": {0: 1.0, 1: 1.0, 2: 1.0, 4: 1.0, 5: 1.0},
    },
    {
        "role": "exclude67_leaveout_4",
        "method": "lingbot_map_stream_sdpa_v118_r77_fresh_exclude67_leaveout_4_seq08",
        "policy": "R77_FRESH_EXCLUDE67_LEAVEOUT_4",
        "source_weights": {0: 1.0, 1: 1.0, 2: 1.0, 3: 1.0, 5: 1.0},
    },
    {
        "role": "exclude67_reliability_weighted_0_5",
        "method": "lingbot_map_stream_sdpa_v118_r77_fresh_exclude67_reliability_weighted_0_5_seq08",
        "policy": "R77_FRESH_EXCLUDE67_RELIABILITY_WEIGHTED_0_5",
        "source_weights": RELIABILITY_0_5,
    },
]
...
def method_yaml(policy: str, source_weights: dict[int, float]) -> str:
    weight_map = {str(frame): float(weight) for frame, weight in sorted(source_weights.items())}
    return "\n".join(
        [
            "model: lingbot_map",
            "env: loger",
...
def build_configs() -> list[dict[str, Any]]:
    ...
    config = CONFIG_DIR / "kitti_lingbot_sdpa_v118_r77_exclude67_source_refinement_seq08.yaml"
    ...
    for gpu, spec in enumerate(METHOD_SPECS):
        ...
        source_weights = {int(k): float(v) for k, v in dict(spec["source_weights"]).items()}
        write_text(CONFIG_DIR / "methods" / f"{method}.yaml", method_yaml(str(spec["policy"]), source_weights))
        ...
        method_rows.append(
            {
                "schema": "acl2_v118tf_stage4_r77_source_refinement_method_row_v1",
                "seq": SEQ,
                "dataset": DATASET,
                "role": spec["role"],
                "method": method,
                "policy": spec["policy"],
                "source_frames": ";".join(str(frame) for frame in sorted(source_weights)),
                "source_weight_map": json.dumps({str(k): v for k, v in sorted(source_weights.items())}, sort_keys=True),
                "config": rel(config),
        ...
        manifest_rows.append(
            {
                "schema": "acl2_v118tf_stage4_r77_source_refinement_run_manifest_row_v1",
                "phase": "cleanup_trace",
                "seq": SEQ,
                "dataset": DATASET,
                "method": method,
    ...
    write_csv(SUMMARY_DIR / "stage4_r77_source_refinement_method_rows.csv", method_rows)
...
def summarize(method_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for spec in method_rows:
        ...
        row = {
            "schema": "acl2_v118tf_stage4_r77_source_refinement_result_row_v1",
            "seq": SEQ,
            "dataset": DATASET,
            "role": spec["role"],
            "method": method,
            "source_frames": spec["source_frames"],
            "source_weight_map": spec["source_weight_map"],
            **metric,
            **action_stats(trace),
        }
        if metric.get("eval_exists"):
            ...
            for label, ref in (("baseline", baseline), ("r74_exclude67", r74), ("r71_original_random", prior_random)):
    ...
    decision = "R77_CONFIG_READY_NOT_RUN"
    if complete:
        if best.get("geometry_gate") and best.get("better_than_r74_exclude67") and best.get("better_than_r71_original_random"):
            decision = "R77_REFINED_SOURCE_GEOMETRY_CANDIDATE_FOUND_REQUIRES_MATCHED_CONTROLS"
        elif best.get("better_than_r74_exclude67"):
            decision = "R77_REFINED_SOURCE_IMPROVES_EXCLUDE67_BUT_GATE_OR_RANDOM_STILL_FAILS"
        elif best.get("geometry_gate"):
            decision = "R77_REFINED_SOURCE_GEOMETRY_GATE_WITHOUT_R74_IMPROVEMENT"
        else:
            decision = "R77_REFINED_SOURCE_NO_GEOMETRY_GATE_REPAIR"
    summary = {
        "schema": "acl2_v118tf_stage4_r77_source_refinement_summary_v1",
        "stage4_r77_decision": decision,
        "complete": complete,
        "global_goal_achieved": False,
        "claim_level": "fresh_seq08_source_refinement_search_only",
        ...
        "result_rows": rows,
        "outputs": {
            "summary": rel(SUMMARY_DIR / "stage4_r77_source_refinement_summary.json"),
            "rows": rel(SUMMARY_DIR / "stage4_r77_source_refinement_rows.csv"),
            "method_rows": rel(SUMMARY_DIR / "stage4_r77_source_refinement_method_rows.csv"),
            "run_manifest": rel(STAGE / "run_manifest.csv"),
        },
    }
    write_csv(SUMMARY_DIR / "stage4_r77_source_refinement_rows.csv", rows)
    write_json(SUMMARY_DIR / "stage4_r77_source_refinement_summary.json", summary)
    return summary
