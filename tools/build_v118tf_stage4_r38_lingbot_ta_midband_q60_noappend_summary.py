#!/usr/bin/env python3
"""Summarize ACL2 v118 Stage4-R38 LingBot TA midband q60 no-append repair."""
...
MANIFEST = STAGE / "summary/stage4_r38_lingbot_ta_midband_q60_noappend_manifest.json"
...
def summarize_rows(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    manifest_rows = {
        ...
        for row in r34s.read_csv(STAGE / "summary/stage4_r38_lingbot_ta_midband_q60_noappend_manifest.csv")
    }
    for method, meta in methods.items():
        for seq in ("00", "02"):
            ...
            rows.append(
                {
                    "schema": "acl2_v118tf_stage4_r38_lingbot_ta_midband_q60_noappend_row_v1",
                    "dataset": DATASET,
                    "seq": seq,
                    "method": method,
                    "branch": "LB-TA",
...
def build_report(summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    lines = [
        "# ACL2 v118-TF Stage4-R38 LB-TA Midband Q60 No-Append Report",
        "",
    ...
    lines += [
        ...
        "## Boundary",
        "",
        "R38 is a post-R36 calibration repair that tests de-clustered median-risk semantic-support selection on both seq00 and seq02. It keeps fixed count, fixed hard no-append runtime surface, and matched temporal-random control.",
    ]
    r34s.write_text(STAGE / "summary/STAGE4_R38_LINGBOT_TA_MIDBAND_Q60_Q60_NOAPPEND_REPORT.md", "\n".join(lines))


def main() -> None:
    manifest = r34s.read_json(MANIFEST)
    rows = summarize_rows(manifest)
    rows_csv = STAGE / "summary/stage4_r38_lingbot_ta_midband_q60_noappend_rows.csv"
    ...
    if global_goal:
        decision = "TA_MIDBAND_Q60_Q60_NOAPPEND_PASS"
    elif candidate_better_all_controls and not baseline_gate:
        decision = "TA_MIDBAND_Q60_Q60_NOAPPEND_CONTROL_PASS_BASELINE_GATE_FAIL"
    else:
        decision = "TA_MIDBAND_Q60_Q60_NOAPPEND_CONTROL_OR_BASELINE_NO_GO"
    summary = {
        "schema": "acl2_v118tf_stage4_r38_lingbot_ta_midband_q60_noappend_summary_v1",
        ...
        "outputs": {
            ...
            "report": r34s.rel(STAGE / "summary/STAGE4_R38_LINGBOT_TA_MIDBAND_Q60_Q60_NOAPPEND_REPORT.md"),
    ...
    r34s.write_json(STAGE / "summary/stage4_r38_lingbot_ta_midband_q60_noappend_summary.json", summary)
    ...
    r34s.add_registry_row(
        {
            ...
            "status": decision,
            "artifact": r34s.rel(STAGE / "summary/stage4_r38_lingbot_ta_midband_q60_noappend_summary.json"),
