#!/usr/bin/env python3
"""ACL2 v28 token-exact semantic risk role router hard-gate audit.
...
present. It also requires the conflict and scale-risk condition streams to be
runtime token-level tensors. It never treats predicted semantics as GT
semantics and never starts candidate rollouts itself.
"""
...
def _phase0_noop_gate(results_root: Path, prefix: str, chunk: int, horizon: int) -> Dict[str, object]:
    reference = _phase0_run_dir(results_root, prefix, "V28_P0_00_H9_REFERENCE", chunk, horizon) / "01.txt"
    rows: List[Dict[str, object]] = []
    ...
    for candidate in PHASE0_CANDIDATES[:4]:
        ...
        row = {
            ...
            "reference_candidate_id": "V28_P0_00_H9_REFERENCE",
...
def _scan_runtime(results_root: Path, prefix: str, chunk: int, horizon: int) -> Dict[str, object]:
    ...
    conflict_token_exact = False
    scale_token_exact = False
    conflict_token_nonempty = False
    scale_token_nonempty = False
    ...
    for candidate in PHASE0_CANDIDATES:
        ...
        for row in _read_jsonl(run_dir / "semantic_memory_path_summary.jsonl"):
            ...
            conflict_token_exact |= bool(row.get("condition_signal_conflict_token_exact", False))
            scale_token_exact |= bool(row.get("condition_signal_scale_risk_token_exact", False))
            conflict_token_nonempty |= bool(row.get("condition_signal_conflict_token_nonempty", False))
            scale_token_nonempty |= bool(row.get("condition_signal_scale_risk_token_nonempty", False))
            condition_levels.append({
                ...
                "condition_signal_conflict_token_exact": row.get("condition_signal_conflict_token_exact"),
                "condition_signal_scale_risk_token_exact": row.get("condition_signal_scale_risk_token_exact"),
                "condition_signal_conflict_token_nonempty": row.get("condition_signal_conflict_token_nonempty"),
                "condition_signal_scale_risk_token_nonempty": row.get("condition_signal_scale_risk_token_nonempty"),
                "condition_signal_conflict_token_count": row.get("condition_signal_conflict_token_count"),
                "condition_signal_scale_risk_token_count": row.get("condition_signal_scale_risk_token_count"),
                "condition_signal_conflict_token_mean": row.get("condition_signal_conflict_token_mean"),
                "condition_signal_conflict_token_p90": row.get("condition_signal_conflict_token_p90"),
                "condition_signal_scale_risk_token_mean": row.get("condition_signal_scale_risk_token_mean"),
                "condition_signal_scale_risk_token_p90": row.get("condition_signal_scale_risk_token_p90"),
    ...
    return {
        ...
        "condition_signal_conflict_token_exact": conflict_token_exact,
        "condition_signal_scale_risk_token_exact": scale_token_exact,
        "condition_signal_conflict_token_nonempty": conflict_token_nonempty,
        "condition_signal_scale_risk_token_nonempty": scale_token_nonempty,
...
def main() -> int:
    ...
    parser.add_argument("--results-root", default="results/kitti01_hmc_v2/acl2_v28_semanticprior_tokenexact_riskconditioned_allmemory_parallel")
    ...
    parser.add_argument("--phase0-prefix", default="V28_P0_SMOKE_R1")
    ...
    runtime_gate = bool(
        ...
        and bool(runtime["condition_signal_conflict_token_exact"])
        and bool(runtime["condition_signal_scale_risk_token_exact"])
        and bool(runtime["condition_signal_conflict_token_nonempty"])
        and bool(runtime["condition_signal_scale_risk_token_nonempty"])
    ...
    if not runtime_gate:
        failures.append({
            ...
            "detail": {
                ...
                "condition_signal_conflict_token_exact": runtime["condition_signal_conflict_token_exact"],
                "condition_signal_scale_risk_token_exact": runtime["condition_signal_scale_risk_token_exact"],
                "condition_signal_conflict_token_nonempty": runtime["condition_signal_conflict_token_nonempty"],
                "condition_signal_scale_risk_token_nonempty": runtime["condition_signal_scale_risk_token_nonempty"],
                "context_empty_source_events": runtime["context_empty_source_events"],
            },
        })
    ...
    summary: Dict[str, object] = {
        "phase": "v28_phase0_token_exact_risk_role_router_audit",
        "uses_gt_semantic": False,
    ...
    report = [
        "# ACL2 v28 Token-Exact Risk Role Router Phase-0 Audit",
        "",
        ...
        f"condition_signal_conflict_token_exact = `{str(runtime['condition_signal_conflict_token_exact']).lower()}`",
        f"condition_signal_scale_risk_token_exact = `{str(runtime['condition_signal_scale_risk_token_exact']).lower()}`",
        f"condition_signal_conflict_token_nonempty = `{str(runtime['condition_signal_conflict_token_nonempty']).lower()}`",
        f"condition_signal_scale_risk_token_nonempty = `{str(runtime['condition_signal_scale_risk_token_nonempty']).lower()}`",
        "",
