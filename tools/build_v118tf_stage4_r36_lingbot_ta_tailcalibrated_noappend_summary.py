#!/usr/bin/env python3
"""Summarize ACL2 v118 Stage4-R36 LingBot TA tail-calibrated no-append repair."""
...
MANIFEST = STAGE / "summary/stage4_r36_lingbot_ta_tailcalibrated_noappend_manifest.json"
...
def summarize_rows(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    manifest_rows = {
        ...
        for row in r34s.read_csv(STAGE / "summary/stage4_r36_lingbot_ta_tailcalibrated_noappend_manifest.csv")
    }
    for method, meta in methods.items():
        for seq in ("00", "02"):
            ...
            rows.append(
                {
                    "schema": "acl2_v118tf_stage4_r36_lingbot_ta_tailcalibrated_noappend_row_v1",
                    "dataset": DATASET,
                    "seq": seq,
                    "method": method,
                    "branch": "LB-TA",
                    ...
                    "tail_q95": manifest_row.get("tail_q95", ""),
...
def build_report(summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    lines = [
        "# ACL2 v118-TF Stage4-R36 LB-TA Tail-Calibrated No-Append Report",
        "",
        ...
        "## Rows",
        "",
        "| seq | role | mode | ATE | rel vs default | action fidelity |",
        "|---|---|---|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            f"| `{row['seq']}` | `{row['role']}` | `{row['selection_mode']}` | "
    ...
    lines += [
        ...
        "## Boundary",
        "",
        "R36 is a post-R35 calibration repair. It uses only R20 support-distribution statistics to choose high-tail or mid-band de-clustered no-append; it does not use ATE, GT, SLAM, external depth, or output post-processing as runtime cue.",
    ]
    r34s.write_text(STAGE / "summary/STAGE4_R36_LINGBOT_TA_TAILCALIBRATED_NOAPPEND_REPORT.md", "\n".join(lines))


def main() -> None:
    manifest = r34s.read_json(MANIFEST)
    rows = summarize_rows(manifest)
    rows_csv = STAGE / "summary/stage4_r36_lingbot_ta_tailcalibrated_noappend_rows.csv"
    ...
    if global_goal:
        decision = "TA_TAILCALIBRATED_NOAPPEND_PASS"
    elif candidate_better_all_controls and not baseline_gate:
        decision = "TA_TAILCALIBRATED_NOAPPEND_CONTROL_PASS_BASELINE_GATE_FAIL"
    else:
        decision = "TA_TAILCALIBRATED_NOAPPEND_CONTROL_OR_BASELINE_NO_GO"
    summary = {
        "schema": "acl2_v118tf_stage4_r36_lingbot_ta_tailcalibrated_noappend_summary_v1",
        ...
        "outputs": {
            ...
            "report": r34s.rel(STAGE / "summary/STAGE4_R36_LINGBOT_TA_TAILCALIBRATED_NOAPPEND_REPORT.md"),
            "runtime": r34s.rel(STAGE / "runtime_full_thread8"),
        },
        "boundary": (
            "Post-R35 support-distribution calibration repair. Success still requires beating "
            "reverse and temporal-random controls plus the default baseline gate."
        ),
    }
    r34s.write_json(STAGE / "summary/stage4_r36_lingbot_ta_tailcalibrated_noappend_summary.json", summary)
    ...
    r34s.add_registry_row(
        {
            ...
            "status": decision,
            "artifact": r34s.rel(STAGE / "summary/stage4_r36_lingbot_ta_tailcalibrated_noappend_summary.json"),
            "notes": "Post-R35 support-distribution tail calibration with reverse and temporal-random controls",
        }
    )
