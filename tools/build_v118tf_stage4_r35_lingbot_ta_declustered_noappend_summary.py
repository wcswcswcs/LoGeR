#!/usr/bin/env python3
"""Summarize ACL2 v118 Stage4-R35 LingBot TA de-clustered no-append repair."""
...
MANIFEST = STAGE / "summary/stage4_r35_lingbot_ta_declustered_noappend_manifest.json"
...
def summarize_rows(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    manifest_rows = {
        ...
        for row in r34s.read_csv(STAGE / "summary/stage4_r35_lingbot_ta_declustered_noappend_manifest.csv")
    }
    for method, meta in methods.items():
        for seq in ("00", "02"):
            ...
            rows.append(
                {
                    "schema": "acl2_v118tf_stage4_r35_lingbot_ta_declustered_noappend_row_v1",
                    "dataset": DATASET,
                    "seq": seq,
                    "method": method,
                    "branch": "LB-TA",
...
def build_report(summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    lines = [
        "# ACL2 v118-TF Stage4-R35 LB-TA Declustered No-Append Report",
        "",
    ...
    lines += [
        ...
        "## Boundary",
        "",
        "R35 keeps R34's fixed 10% hard no-append budget but adds a local de-cluster gap before runtime. This tests whether R34's seq02 failure is driven by consecutive keyframe drops rather than semantic carrier value.",
    ]
    r34s.write_text(STAGE / "summary/STAGE4_R35_LINGBOT_TA_DECLUSTERED_NOAPPEND_REPORT.md", "\n".join(lines))


def main() -> None:
    manifest = r34s.read_json(MANIFEST)
    rows = summarize_rows(manifest)
    rows_csv = STAGE / "summary/stage4_r35_lingbot_ta_declustered_noappend_rows.csv"
    ...
    if global_goal:
        decision = "TA_DECLUSTERED_NOAPPEND_PASS"
    elif candidate_better_all_controls and not baseline_gate:
        decision = "TA_DECLUSTERED_NOAPPEND_CONTROL_PASS_BASELINE_GATE_FAIL"
    else:
        decision = "TA_DECLUSTERED_NOAPPEND_CONTROL_OR_BASELINE_NO_GO"
    summary = {
        "schema": "acl2_v118tf_stage4_r35_lingbot_ta_declustered_noappend_summary_v1",
        ...
        "outputs": {
            ...
            "report": r34s.rel(STAGE / "summary/STAGE4_R35_LINGBOT_TA_DECLUSTERED_NOAPPEND_REPORT.md"),
            "runtime": r34s.rel(STAGE / "runtime_full_thread8"),
        },
        "boundary": (
            "Fixed-budget de-clustered no-append tests R34's consecutive-drop failure mode. "
            "It keeps the same force_non_keyframe runtime surface and matched controls."
        ),
    }
    r34s.write_json(STAGE / "summary/stage4_r35_lingbot_ta_declustered_noappend_summary.json", summary)
    ...
    r34s.add_registry_row(
        {
            ...
            "status": decision,
            "artifact": r34s.rel(STAGE / "summary/stage4_r35_lingbot_ta_declustered_noappend_summary.json"),
            "notes": "Fixed-budget de-clustered guarded semantic-support hard no-append with reverse and temporal-random controls",
        }
    )
