#!/usr/bin/env python3
"""Build ACL2 v118 Stage4-R36 LingBot TA tail-calibrated no-append configs."""
...
STAGE = RESULT_ROOT / "stage4_r36_lingbot_ta_tailcalibrated_noappend"
...
TEMPORAL_BUCKETS = 4
MIN_FRAME_GAP = 60
TAIL_Q95_HIGH_THRESHOLD = 0.58
MIDBAND_QUANTILE = 0.50
RANDOM_SEED = 11836

METHODS = {
    "lingbot_map_stream_flashinfer_v118_r36_ta5_tailcalibrated_drop": {
        "policy": "TA5_TAILCALIBRATED_SEMANTIC_DROP",
        "role": "candidate",
        "description": "drop high-risk frames only when absolute tail is high, otherwise drop de-clustered mid-risk frames",
    },
    "lingbot_map_stream_flashinfer_v118_r36_ta5_tailcalibrated_reverse": {
        "policy": "TA5_TAILCALIBRATED_REVERSE_DROP",
        "role": "reverse_control",
        "description": "drop the opposite calibrated risk band with the same de-cluster gap",
    },
    "lingbot_map_stream_flashinfer_v118_r36_ta5_tailcalibrated_temporal_random": {
        "policy": "TA5_TAILCALIBRATED_MATCHED_TEMPORAL_RANDOM_DROP",
...
def selected_indices(rows_by_frame: dict[int, dict[str, Any]], seq: str) -> tuple[dict[str, list[int]], dict[str, Any]]:
    ...
    q50 = quantile(values, MIDBAND_QUANTILE)
    q95 = quantile(values, 0.95)
    ranked_high = sorted(eligible, key=lambda frame: (-risk[frame], frame))
    ranked_low = sorted(eligible, key=lambda frame: (risk[frame], frame))
    ranked_mid = sorted(eligible, key=lambda frame: (abs(risk[frame] - q50), frame))
    if q95 >= TAIL_Q95_HIGH_THRESHOLD:
        candidate = r35.greedy_gap_select(ranked_high, budget, MIN_FRAME_GAP)
        reverse = r35.greedy_gap_select(ranked_low, budget, MIN_FRAME_GAP)
        mode = "high_tail"
    else:
        ...
        mode = "midband_tail_compressed"
    return (
        {
            "lingbot_map_stream_flashinfer_v118_r36_ta5_tailcalibrated_drop": candidate,
            "lingbot_map_stream_flashinfer_v118_r36_ta5_tailcalibrated_reverse": reverse,
            "lingbot_map_stream_flashinfer_v118_r36_ta5_tailcalibrated_temporal_random": temporal_matched_random_gap(
                eligible, candidate, seq
            ),
        },
        {"tail_q95": q95, "risk_q50": q50, "selection_mode": mode},
    )
...
def write_main_config(config_dir: Path) -> None:
    (config_dir / "kitti_lingbot_flashinfer_r36_ta_tailcalibrated_noappend_full_reuse_v105gt.yaml").write_text(
        "\n".join(
            [
...
def write_manifest(rows: list[dict[str, Any]], seq_maps_by_method: dict[str, dict[str, list[int]]]) -> None:
    ...
    csv_path = out / "stage4_r36_lingbot_ta_tailcalibrated_noappend_manifest.csv"
    ...
    payload = {
        "schema": "acl2_v118tf_stage4_r36_lingbot_ta_tailcalibrated_noappend_manifest_v1",
        "stage": r34.rel(STAGE),
        "dataset": DATASET,
        ...
        "tail_q95_high_threshold": TAIL_Q95_HIGH_THRESHOLD,
    ...
    (out / "stage4_r36_lingbot_ta_tailcalibrated_noappend_manifest.json").write_text(
...
def main() -> None:
    ...
    for seq, rows_by_frame in sorted(support.items()):
        ...
        candidate_frames = selections["lingbot_map_stream_flashinfer_v118_r36_ta5_tailcalibrated_drop"]
        ...
        for method, frames in selections.items():
            ...
            manifest_rows.append(
                {
                    "schema": "acl2_v118tf_stage4_r36_lingbot_ta_tailcalibrated_noappend_manifest_row_v1",
                    "seq": seq,
                    "method": method,
        ...
        json.dumps(
            {
                "stage": r34.rel(STAGE),
                "config": r34.rel(
                    config_dir / "kitti_lingbot_flashinfer_r36_ta_tailcalibrated_noappend_full_reuse_v105gt.yaml"
                ),
