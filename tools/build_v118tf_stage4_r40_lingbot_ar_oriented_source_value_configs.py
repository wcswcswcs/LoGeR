#!/usr/bin/env python3
"""Build ACL2 v118 Stage4-R40 LingBot AR oriented source-value configs."""
...
METHODS = {
    f"{METHOD_PREFIX}_ar7_corr_oriented_source_value": {
        "policy": "AR7_CORR_ORIENTED_SOURCE_VALUE",
        "role": "candidate",
        "weight_mode": "corr_oriented",
        "description": "use direct weights when internal/semantic anchor cue correlation is non-positive, otherwise reciprocal weights",
    },
    f"{METHOD_PREFIX}_ar7_opposite_sign_source_value_control": {
        "policy": "AR7_OPPOSITE_SIGN_SOURCE_VALUE_CONTROL",
        "role": "opposite_sign_control",
        "weight_mode": "opposite_sign",
        "description": "matched opposite orientation of the correlation rule",
    },
    f"{METHOD_PREFIX}_ar7_rotated_source_value_control": {
        "policy": "AR7_ROTATED_SOURCE_VALUE_CONTROL",
        "role": "rotated_source_control",
        "weight_mode": "rotated_source",
        "description": "same oriented weights rotated across fixed anchor source frames",
    },
}
...
def main() -> None:
    ...
    for method_base, meta in METHODS.items():
        ...
        for seq in SEQS:
            ...
            manifest_rows.append(
                {
                    "schema": f"acl2_v118tf_stage4_{STAGE_TAG}_lingbot_ar_oriented_source_value_manifest_row_v1",
                    "seq": seq,
    ...
    weight_rows_name = f"stage4_{STAGE_TAG}_lingbot_ar_oriented_source_value_weight_rows.csv"
    manifest_csv_name = f"stage4_{STAGE_TAG}_lingbot_ar_oriented_source_value_manifest.csv"
    ...
    manifest = {
        "schema": f"acl2_v118tf_stage4_{STAGE_TAG}_lingbot_ar_oriented_source_value_manifest_v1",
        ...
        "branch": "LB-AR",
        "operation": "Anchor read source-value correlation-oriented repair",
        ...
        "r30_reference": "results/acl2_v118tf_operation_specific_semantic_carrier_calibration/stage4_r30_lingbot_ar_source_value_scaling/summary/stage4_r30_lingbot_ar_anchor_read_summary.json",
        "boundary": (
            "R40 tests a predeclared internal/semantic correlation orientation rule for source-value scaling. "
            "A pass requires beating opposite-sign and rotated-source controls on 00/02 and then fresh 01/05 validation; "
            "this dev run alone is not a global success claim."
        ),
