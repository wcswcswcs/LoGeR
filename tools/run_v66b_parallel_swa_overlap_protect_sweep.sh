...
PROTECT_SCALE="${PROTECT_SCALE:-0.25}"
RUN_TAG="${RUN_TAG:-FIX2_P025}"
...
COMMON_ENV=(
  ...
  SEMANTIC_ROLE_SWA_PROTECT_SCALE="$PROTECT_SCALE"
  READ_PATH=frame
...
run_job 0 "${RUN_PREFIX}_GATE_PROTECT_SEM_RHO050${RUN_SUFFIX}" \
  ENABLE_SWA_OVERLAP_SOURCE_GATE=1 \
  SWA_OVERLAP_SOURCE_GATE_RHO=0.50 \
  SEMANTIC_ROLE_CONTROL_MODE=none

run_job 1 "${RUN_PREFIX}_GATE_PROTECT_RANDOM_RHO050${RUN_SUFFIX}" \
  ENABLE_SWA_OVERLAP_SOURCE_GATE=1 \
  SWA_OVERLAP_SOURCE_GATE_RHO=0.50 \
  SEMANTIC_ROLE_CONTROL_MODE=random_same_mass \
  SEMANTIC_ROLE_CONTROL_SEED=12345

run_job 2 "${RUN_PREFIX}_GATE_PROTECT_SEM_RHO075${RUN_SUFFIX}" \
  ENABLE_SWA_OVERLAP_SOURCE_GATE=1 \
  SWA_OVERLAP_SOURCE_GATE_RHO=0.75 \
  SEMANTIC_ROLE_CONTROL_MODE=none

run_job 3 "${RUN_PREFIX}_GATE_PROTECT_RANDOM_RHO075${RUN_SUFFIX}" \
  ENABLE_SWA_OVERLAP_SOURCE_GATE=1 \
  SWA_OVERLAP_SOURCE_GATE_RHO=0.75 \
  SEMANTIC_ROLE_CONTROL_MODE=random_same_mass \
  SEMANTIC_ROLE_CONTROL_SEED=12345
...
echo "[$(date '+%F %T')] all v66b phase13 SWA protect-sweep jobs finished"
