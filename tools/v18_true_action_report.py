#!/usr/bin/env python3
"""Aggregate ACL2 v18 true-action causal-fork horizon rollouts.
...
FAMILY_BY_CANDIDATE = {
    "K1_H9": "baseline",
    "PZBASIS_01_CONTINUITY_BOOST_W0": "true_post_zp_basis",
    "PZBASIS_02_HARM_SUPPRESS_W0": "true_post_zp_basis",
    "PZBASIS_03_HARM_SUPPRESS_W0W2": "true_post_zp_basis",
    "PZBASIS_04_SCALE_BASIS_SUPPRESS_W0": "true_post_zp_basis",
    "PZBASIS_05_OVERLAP_BASIS_BOOST_W0": "true_post_zp_basis",
    "PZBASIS_06_CONTINUITY_PLUS_HARM_W0": "true_post_zp_basis",
    "PZBASIS_07_LOWRANK_BASIS_RANK4": "true_post_zp_basis",
    "PZBASIS_08_LOWRANK_BASIS_RANK8": "true_post_zp_basis",
    "AUXGEO_TRUE_01_OVERLAP_POINTMAP_V_W0": "true_auxgeo_replay",
    "AUXGEO_TRUE_02_OVERLAP_POINTMAP_KV_W0": "true_auxgeo_replay",
    "AUXGEO_TRUE_03_OVERLAP_SCALE_PROXY_W0": "true_auxgeo_replay",
    "AUXGEO_TRUE_04_OVERLAP_SCALE_PROXY_W0W2": "true_auxgeo_replay",
    "AUXGEO_TRUE_05_STRUCTURE_ONLY_OVERLAP_W0": "true_auxgeo_replay",
    "AUXGEO_TRUE_06_LOWD_STRUCTURE_OVERLAP_W0": "true_auxgeo_replay",
    "DLTRUE_01_SHORT_HARM_ONLY_W0": "true_dual_bank",
    "DLTRUE_02_SHORT_BODY_HARM_LONG_CONTINUITY": "true_dual_bank",
    "DLTRUE_03_SHORT_SCALE_CORRECTION_LONG_STRUCTURE": "true_dual_bank",
    "DLTRUE_04_SHORT_OVERLAP_CORRECTION_LONG_NATIVE": "true_dual_bank",
    "DLTRUE_05_SHORT_DECAY_FAST_K2": "true_dual_bank",
    "DLTRUE_06_SHORT_DECAY_SLOW_K5": "true_dual_bank",
    "DLTRUE_07_RESET_BOUND_SHORT_CLEAR": "true_dual_bank",
    "DLTRUE_08_EXIT_WEAK_SHORT_HANDOFF": "true_dual_bank",
}
...
def _is_deployable_ttt_write_candidate(candidate_id: str) -> bool:
    return not candidate_id.startswith("K1_")
...
def _gate_summary(rows: List[Dict[str, object]]) -> Dict[str, object]:
    candidates = [row for row in rows if row.get("candidate_id") != "K1_H9" and int(row.get("horizon", 0)) in {10, 15}]
    best_ate = min(
        candidates,
    ...
    pass_gate = (
        ...
        and _to_float(best_ate.get("ATE_delta_vs_H9")) <= -3.0
    ...
    weak_gate = (
        ...
        and _to_float(best_ate.get("ATE_delta_vs_H9")) <= -1.0
        and no_downstream_regression(best_ate)
    )
    status = "pass" if pass_gate else ("weak" if weak_gate else "fail")
    return {
        "phase": "v18 true-action candidate bank",
        "status": status,
        "best_h10_h15_ATE_candidate": best_ate.get("candidate_id") if best_ate else "",
        "best_h10_h15_ATE_chunk": best_ate.get("chunk_id") if best_ate else "",
        "best_h10_h15_ATE_horizon": best_ate.get("horizon") if best_ate else "",
        "best_h10_h15_ATE_delta_vs_H9": best_ate.get("ATE_delta_vs_H9") if best_ate else float("nan"),
        ...
        "selector_allowed": status == "pass",
        "full_online_validation_allowed": False,
        "gate_rule": "pass if h10/h15 ATE delta <= -3m or [200,300) delta <= -5m with [400,600) proxy <= +1m",
    }
...
def _write_markdown(path: Path, rows: List[Dict[str, object]], summary: Mapping[str, object], best_rows: List[Dict[str, object]]) -> None:
    lines = [
        ...
        "",
        f"Best h10/h15 ATE delta: `{summary.get('best_h10_h15_ATE_delta_vs_H9')}` "
        f"({summary.get('best_h10_h15_ATE_candidate')} chunk {summary.get('best_h10_h15_ATE_chunk')} h{summary.get('best_h10_h15_ATE_horizon')})",
        "",
        ...
        "",
        f"Selector allowed: `{str(summary.get('selector_allowed')).lower()}`.",
        "",
...
def main() -> None:
    ...
    parser.add_argument(
        "--run-prefix",
        default="V18",
        help="Run directory prefix used by tools/run_v18_candidate_rollout.sh.",
    )
    ...
    horizons = args.horizon or [5, 8, 10, 15]
    ...
    _write_csv(out_dir / "short_rollout_metrics_h5_h8_h10_h15.csv", rows)
    _write_json(out_dir / "short_rollout_metrics_h5_h8_h10_h15.json", rows)
    delta_rows = [
        {
    ...
    _write_json(out_dir / "true_action_gate_summary.json", summary)
    _write_markdown(out_dir / "acl2_v18_true_action_report.md", rows, summary[0], best_rows)
