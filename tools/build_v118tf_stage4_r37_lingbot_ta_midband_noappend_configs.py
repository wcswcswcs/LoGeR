#!/usr/bin/env python3
"""Build ACL2 v118 Stage4-R37 LingBot TA midband de-clustered no-append configs."""
...
METHODS = {
    "lingbot_map_stream_flashinfer_v118_r37_ta5_midband_drop": {
        "policy": "TA5_MIDBAND_SEMANTIC_DROP",
        "role": "candidate",
        "description": "drop de-clustered frames closest to the sequence median guarded semantic-support risk",
    },
    "lingbot_map_stream_flashinfer_v118_r37_ta5_highrisk_reverse": {
        "policy": "TA5_MIDBAND_HIGH_RISK_REVERSE_DROP",
        "role": "reverse_control",
        "description": "drop high guarded-risk frames with the same de-cluster gap",
    },
    "lingbot_map_stream_flashinfer_v118_r37_ta5_midband_temporal_random": {
        "policy": "TA5_MIDBAND_MATCHED_TEMPORAL_RANDOM_DROP",
...
def selected_indices(rows_by_frame: dict[int, dict[str, Any]], seq: str) -> tuple[dict[str, list[int]], dict[str, Any]]:
    ...
    q50 = r36.quantile(values, MIDBAND_QUANTILE)
    ...
    return (
        {
            "lingbot_map_stream_flashinfer_v118_r37_ta5_midband_drop": candidate,
            "lingbot_map_stream_flashinfer_v118_r37_ta5_highrisk_reverse": reverse,
            "lingbot_map_stream_flashinfer_v118_r37_ta5_midband_temporal_random": temporal_matched_random_gap(
                eligible, candidate, seq
            ),
        },
        {"risk_q50": q50, "selection_mode": "midband_all_sequences"},
    )
...
def write_main_config(config_dir: Path) -> None:
    (config_dir / "kitti_lingbot_flashinfer_r37_ta_midband_noappend_full_reuse_v105gt.yaml").write_text(
        "\n".join(
            [
...
def write_manifest(rows: list[dict[str, Any]], seq_maps_by_method: dict[str, dict[str, list[int]]]) -> None:
    ...
    csv_path = out / "stage4_r37_lingbot_ta_midband_noappend_manifest.csv"
    ...
    payload = {
        "schema": "acl2_v118tf_stage4_r37_lingbot_ta_midband_noappend_manifest_v1",
        "stage": r34.rel(STAGE),
        "dataset": DATASET,
    ...
    (out / "stage4_r37_lingbot_ta_midband_noappend_manifest.json").write_text(
...
def main() -> None:
    ...
    for seq, rows_by_frame in sorted(support.items()):
        ...
        candidate_frames = selections["lingbot_map_stream_flashinfer_v118_r37_ta5_midband_drop"]
        ...
        for method, frames in selections.items():
            ...
            manifest_rows.append(
                {
                    "schema": "acl2_v118tf_stage4_r37_lingbot_ta_midband_noappend_manifest_row_v1",
                    "seq": seq,
                    "method": method,
    ...
    print(
        json.dumps(
            {
                "stage": r34.rel(STAGE),
                "config": r34.rel(config_dir / "kitti_lingbot_flashinfer_r37_ta_midband_noappend_full_reuse_v105gt.yaml"),
