#!/usr/bin/env python3
"""Run v92 Phase4 SWA semantic-policy route control smokes."""
...
DEFAULT_OUTPUT_ROOT = ROOT / "phase4_swa_semantic_policy_route_audit"
DEFAULT_MASK_CSV = ROOT / "phase4_swa_policy_route_masks/v92_swa_policy_route_mask_positions.csv"
...
VARIANTS = {
    "semantic": "v92_semantic_shuffle_pair_mask",
    "component": "v92_component_shuffle_pair_mask",
    "regime": "v92_regime_shuffle_pair_mask",
    "geometry": "v92_geometry_only_risk_pair_mask",
}
PAIR_CASE = "P9_54_ATTENTION_BIAS_V92_POLICY_PAIR_MASS_AUDIT_LAST"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--variants", default="semantic,component,regime,geometry")
...
def _build_job(args: argparse.Namespace, *, seq: str, variant_key: str, gpu: int) -> dict[str, Any]:
    variant = VARIANTS[variant_key]
    chunk = int(SEQ_SPECS[seq]["chunk"])
    out_root = args.output_root / f"{variant_key}_shuffle_controls" / f"seq{seq}_chunk{chunk:02d}_v92_{variant_key}_pair"
    cmd = [
        str(args.conda),
        "run",
        "-n",
...
def _run_job(job: dict[str, Any]) -> dict[str, Any]:
    ...
    (out_root / "v92_outer_run.log").write_text(proc.stdout, encoding="utf-8")
    updated = dict(job)
    updated.update(
        {
            "returncode": int(proc.returncode),
            "duration_sec": float(end - start),
            "outer_run_log": str(out_root / "v92_outer_run.log"),
...
def main() -> None:
    ...
    for variant_key in variant_keys:
        for seq in seqs:
            jobs.append(_build_job(args, seq=seq, variant_key=variant_key, gpu=gpus[cursor % len(gpus)]))
            cursor += 1

    args.output_root.mkdir(parents=True, exist_ok=True)
    manifest_path = args.output_root / "v92_swa_control_run_manifest.json"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as pool:
        ...
        for fut in concurrent.futures.as_completed(futures):
            ...
            print(
                f"[gpu{result['gpu']}] variant={result['variant_key']} seq={result['seq']} "
                f"returncode={result['returncode']} duration={result['duration_sec']:.1f}s"
            )
            manifest["jobs"] = results + [job for job in jobs if job not in results]
            manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True, default=str) + "\n", encoding="utf-8")

    failed = [
        {"seq": row["seq"], "variant": row["variant_key"], "returncode": row["returncode"]}
        for row in results
