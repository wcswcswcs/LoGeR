#!/usr/bin/env python3
"""Build ACL2 v118 Stage4-R42 holdout token semantic tensors for seq01/05."""
...
STAGE = RESULT_ROOT / "stage4_r42_lingbot_ar_token_gated_oriented_source_value_holdout"
...
def main() -> int:
    ...
    frame_universe = {str(k).zfill(2): int(v) for k, v in dict(summary["frame_universe_by_seq"]).items()}
    ...
    missing = [seq for seq in SEQUENCES if seq not in frame_universe]
    if missing:
        raise RuntimeError(f"stage2 summary missing holdout sequences: {missing}")
    ...
    with TOKEN_ROWS.open("r", encoding="utf-8", newline="") as handle:
        ...
        for row in reader:
            scanned_rows += 1
            seq = str(row.get("seq_id", "")).zfill(2)
    ...
    report_lines = [
        "# ACL2 v118 Stage4-R42 Holdout Token Semantic Tensor Build",
        "",
    ...
    report_lines += [
        "",
        "Boundary: this build only materializes frozen token semantic channels for fresh 01/05 validation. "
        "It does not inspect or tune against holdout ATE.",
    ]
    report_path.write_text("\n".join(report_lines).rstrip() + "\n", encoding="utf-8")

    out_summary = {
        "schema": "acl2_v118tf_stage4_r42_holdout_token_semantic_build_summary_v1",
        ...
        "scanned_token_rows": scanned_rows,
