#!/usr/bin/env python3
"""Summarize ACL2 v118 Stage4-R24 bin-balanced LingBot anchor repair."""
...
MANIFEST = STAGE / "summary/stage4_r24_lingbot_ai_bin_anchor_manifest.json"
...
def summarize_rows(manifest: dict) -> list[dict]:
    ...
    for method, meta in manifest.get("methods", {}).items():
        for seq in ("00", "02"):
            ...
            rows.append(
                {
                    "schema": "acl2_v118tf_stage4_r24_lingbot_ai_bin_anchor_row_v1",
                    "dataset": DATASET,
                    "seq": seq,
                    "method": method,
                    "branch": "LB-AI",
...
def main() -> None:
    ...
    if not complete:
        decision = "AI_BIN_ANCHOR_RUNTIME_INCOMPLETE"
    elif not action_fidelity:
        decision = "AI_BIN_ANCHOR_ACTION_FIDELITY_NO_GO"
    elif candidate_better_all_controls and baseline_gate:
        decision = "AI_BIN_ANCHOR_PROMOTION_CANDIDATE_FOUND"
    else:
        decision = "AI_BIN_ANCHOR_CONTROL_OR_BASELINE_NO_GO"

    out = STAGE / "summary"
    summary = {
        "schema": "acl2_v118tf_stage4_r24_lingbot_ai_bin_anchor_summary_v1",
        "stage4_r24_decision": decision,
        ...
        "boundary": (
            "R24 tests LB-AI bin-balanced first32 anchor initialization. Each policy selects one scale frame per four-frame bin, "
            "so temporal/bin coverage is matched across candidate and controls."
        ),
        "outputs": {
            "rows": str((out / "stage4_r24_lingbot_ai_bin_anchor_rows.csv").relative_to(ROOT)),
            "summary": str((out / "stage4_r24_lingbot_ai_bin_anchor_summary.json").relative_to(ROOT)),
            "report": str((out / "STAGE4_R24_LINGBOT_AI_BIN_ANCHOR_REPORT.md").relative_to(ROOT)),
        },
    }
    out.mkdir(parents=True, exist_ok=True)
    write_csv(out / "stage4_r24_lingbot_ai_bin_anchor_rows.csv", rows)
    (out / "stage4_r24_lingbot_ai_bin_anchor_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    report = [
        "# Stage4-R24 LingBot AI Bin Anchor Summary",
        "",
        f"decision: `{decision}`",
    ...
    (out / "STAGE4_R24_LINGBOT_AI_BIN_ANCHOR_REPORT.md").write_text(
        "\n".join(report) + "\n",
        encoding="utf-8",
    )
