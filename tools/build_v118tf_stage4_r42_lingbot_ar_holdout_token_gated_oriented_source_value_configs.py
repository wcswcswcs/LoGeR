#!/usr/bin/env python3
"""Build ACL2 v118 Stage4-R42 fresh 01/05 token-gated source-value configs."""
...
METHODS = {
    f"{METHOD_PREFIX}_ar8_holdout_token_frame_semantic_source_value": {
        "policy": "AR8_HOLDOUT_TOKEN_FRAME_SEMANTIC_SOURCE_VALUE",
        "role": "candidate",
        "weight_mode": "corr_oriented",
        "token_weight_mode": "risk_suppress_plus_stable_x_frame",
        "description": "fresh 01/05 validation of the frozen R41 correlation-oriented frame weights multiplied by token-level semantic weights",
    },
    f"{METHOD_PREFIX}_ar8_holdout_opposite_frame_token_source_value_control": {
        "policy": "AR8_HOLDOUT_OPPOSITE_FRAME_TOKEN_SOURCE_VALUE_CONTROL",
        "role": "opposite_frame_control",
        "weight_mode": "opposite_sign",
        "token_weight_mode": "risk_suppress_plus_stable_x_frame",
        "description": "fresh 01/05 matched token semantic weights with opposite frame orientation",
    },
    f"{METHOD_PREFIX}_ar8_holdout_rotated_frame_token_source_value_control": {
        "policy": "AR8_HOLDOUT_ROTATED_FRAME_TOKEN_SOURCE_VALUE_CONTROL",
        "role": "rotated_frame_control",
        "weight_mode": "rotated_source",
        "token_weight_mode": "risk_suppress_plus_stable_x_frame",
        "description": "fresh 01/05 matched token semantic weights with rotated oriented frame assignment",
    },
    f"{METHOD_PREFIX}_ar8_holdout_token_reverse_source_value_control": {
        "policy": "AR8_HOLDOUT_TOKEN_REVERSE_SOURCE_VALUE_CONTROL",
        "role": "token_reverse_control",
        "weight_mode": "corr_oriented",
        "token_weight_mode": "reverse_risk_x_frame",
        "description": "fresh 01/05 matched oriented frame weights with reverse token semantic formula",
    },
    f"{METHOD_PREFIX}_ar8_holdout_token_random_source_value_control": {
        "policy": "AR8_HOLDOUT_TOKEN_RANDOM_SOURCE_VALUE_CONTROL",
        "role": "token_random_control",
        "weight_mode": "corr_oriented",
        "token_weight_mode": "same_magnitude_random_logit_x_frame",
        "description": "fresh 01/05 matched oriented frame weights with deterministic same-magnitude random token signs",
    },
}
...
def main() -> None:
    ...
    for method_base, meta in METHODS.items():
        ...
        for seq in SEQS:
            ...
            manifest_rows.append(
                {
                    "schema": f"acl2_v118tf_stage4_{STAGE_TAG}_lingbot_ar_holdout_token_gated_source_value_manifest_row_v1",
                    "seq": seq,
    ...
    manifest = {
        "schema": f"acl2_v118tf_stage4_{STAGE_TAG}_lingbot_ar_holdout_token_gated_source_value_manifest_v1",
        "stage": str(STAGE.relative_to(ROOT)),
        "stage_tag": STAGE_TAG,
        "backend_label": BACKEND_LABEL,
        "use_sdpa": True,
        "action_mode": base.ACTION_MODE,
        "intervention_form": f"{base.INTERVENTION_FORM}_token_gated_holdout",
        "dataset": DATASET,
        ...
        "branch": "LB-AR",
        "operation": "Fresh 01/05 validation of R41 token-gated source-value correlation-oriented repair",
        ...
        "boundary": (
            "R42 is a fresh 01/05 validation of the frozen R41 rule. It uses the same orientation rule, "
            "same token modes, same source frames 0-7, and fresh 01/05 support/internal/token tensors. "
            "It must not be retuned after looking at holdout ATE."
        ),
