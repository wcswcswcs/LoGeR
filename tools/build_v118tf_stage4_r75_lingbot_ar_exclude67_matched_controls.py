#!/usr/bin/env python3
"""Build and summarize R75 matched controls for R74 exclude-6/7 candidate."""
...
STAGE = RESULT_ROOT / "stage4_r75_lingbot_ar_fresh_exclude67_matched_controls_08"
...
R74_CANDIDATE = "lingbot_map_stream_sdpa_v118_r74_fresh_risk_only_exclude_6_7_seq08"
...
METHOD_SPECS = [
    {
        "role": "exclude67_permuted_control",
        "method": "lingbot_map_stream_sdpa_v118_r75_fresh_exclude67_permuted_control_seq08",
        "policy": "R75_FRESH_EXCLUDE67_PERMUTED_CONTROL",
        "token_weight_mode": "risk_suppress_only_permuted_logit_x_frame",
    },
    {
        "role": "exclude67_signshuffle_control",
        "method": "lingbot_map_stream_sdpa_v118_r75_fresh_exclude67_signshuffle_control_seq08",
        "policy": "R75_FRESH_EXCLUDE67_SIGN_SHUFFLE_CONTROL",
...
def build_configs() -> list[dict[str, Any]]:
    ...
    config = CONFIG_DIR / "kitti_lingbot_sdpa_v118_r75_exclude67_matched_controls_seq08.yaml"
    ...
    for gpu, spec in enumerate(METHOD_SPECS):
        ...
        method_rows.append(
            {
                "schema": "acl2_v118tf_stage4_r75_exclude67_control_method_row_v1",
                **spec,
                "seq": SEQ,
                "dataset": DATASET,
        ...
        manifest_rows.append(
            {
                "schema": "acl2_v118tf_stage4_r75_exclude67_control_run_manifest_row_v1",
                "phase": "cleanup_trace",
                "seq": SEQ,
                "dataset": DATASET,
                "method": method,
    ...
    write_csv(SUMMARY_DIR / "stage4_r75_exclude67_control_method_rows.csv", method_rows)
...
def summarize(method_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    candidate = read_metric(R74_CANDIDATE)
    ...
    for spec in method_rows:
        ...
        row = {
            "schema": "acl2_v118tf_stage4_r75_exclude67_control_result_row_v1",
            "seq": SEQ,
            "dataset": DATASET,
            "role": spec["role"],
            "method": method,
        ...
        if metric.get("eval_exists") and candidate.get("eval_exists"):
    ...
    beats_controls = complete and all(bool(row.get("candidate_better_control")) for row in rows)
    beats_prior_random = bool(candidate.get("eval_exists") and prior_random.get("eval_exists") and candidate["ate"] < prior_random["ate"])
    decision = "R75_CONFIG_READY_NOT_RUN"
    if complete:
        if beats_controls and beats_prior_random:
            decision = "R75_EXCLUDE67_MATCHED_CONTROLS_PASS_SEQ08"
        elif beats_controls:
            decision = "R75_EXCLUDE67_BEATS_MATCHED_CONTROLS_BUT_NOT_PRIOR_RANDOM"
        else:
            decision = "R75_EXCLUDE67_MATCHED_CONTROL_STILL_COMPETITIVE"
    summary = {
        "schema": "acl2_v118tf_stage4_r75_exclude67_control_summary_v1",
        "stage4_r75_decision": decision,
        "complete": complete,
        "global_goal_achieved": False,
        "claim_level": "fresh_seq08_exclude67_matched_control_validation_only",
        "baseline": baseline,
        "r74_candidate": candidate,
        "r71_original_random_control": prior_random,
        "beats_matched_controls": beats_controls,
        "beats_prior_random": beats_prior_random,
        "result_rows": rows,
        "outputs": {
            "summary": rel(SUMMARY_DIR / "stage4_r75_exclude67_control_summary.json"),
            "rows": rel(SUMMARY_DIR / "stage4_r75_exclude67_control_rows.csv"),
            "method_rows": rel(SUMMARY_DIR / "stage4_r75_exclude67_control_method_rows.csv"),
            "run_manifest": rel(STAGE / "run_manifest.csv"),
        },
    }
    write_csv(SUMMARY_DIR / "stage4_r75_exclude67_control_rows.csv", rows)
    write_json(SUMMARY_DIR / "stage4_r75_exclude67_control_summary.json", summary)
    return summary
