#!/usr/bin/env python3
"""Summarize ACL2 v118 Stage4-R39 LingBot TA midband context-special repair."""
...
import build_v118tf_stage4_r33_lingbot_ta_soft_context_gate_summary as r33s
...
MANIFEST = STAGE / "summary/stage4_r39_lingbot_ta_midband_context_special_manifest.json"
...
def summarize_rows(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    manifest_rows = {
        ...
        for row in r34s.read_csv(STAGE / "summary/stage4_r39_lingbot_ta_midband_context_special_manifest.csv")
    }
    for method, meta in methods.items():
        for seq in ("00", "02"):
            ...
            rows.append(
                {
                    "schema": "acl2_v118tf_stage4_r39_lingbot_ta_midband_context_special_row_v1",
                    "dataset": DATASET,
                    "seq": seq,
                    "method": method,
                    "branch": "LB-TA",
                    ...
                    "risk_target_quantile": manifest_row.get("risk_target_quantile", ""),
                    "risk_target": manifest_row.get("risk_target", ""),
                    "context_only_special_count": len(selected),
                    ...
                    **r33s.action_trace_stats(action_path, selected, tuple()),
...
def build_report(summary: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    lines = [
        "# ACL2 v118-TF Stage4-R39 LB-TA Midband Context-Special Report",
        "",
    ...
    lines += [
        ...
        "## Boundary",
        "",
        "R39 moves the R37 midband de-clustered semantic-support carrier set from hard no-append into the context-only-special operation surface. It keeps fixed count, fixed min-gap, high-risk reverse control, and matched temporal-random control.",
    ]
    r34s.write_text(STAGE / "summary/STAGE4_R39_LINGBOT_TA_MIDBAND_CONTEXT_SPECIAL_REPORT.md", "\n".join(lines))


def main() -> None:
    manifest = r34s.read_json(MANIFEST)
    rows = summarize_rows(manifest)
    rows_csv = STAGE / "summary/stage4_r39_lingbot_ta_midband_context_special_rows.csv"
    ...
    if global_goal:
        decision = "TA_MIDBAND_CONTEXT_SPECIAL_PASS"
    elif candidate_better_all_controls and not baseline_gate:
        decision = "TA_MIDBAND_CONTEXT_SPECIAL_CONTROL_PASS_BASELINE_GATE_FAIL"
    else:
        decision = "TA_MIDBAND_CONTEXT_SPECIAL_CONTROL_OR_BASELINE_NO_GO"
    summary = {
        "schema": "acl2_v118tf_stage4_r39_lingbot_ta_midband_context_special_summary_v1",
        ...
        "outputs": {
            ...
            "report": r34s.rel(STAGE / "summary/STAGE4_R39_LINGBOT_TA_MIDBAND_CONTEXT_SPECIAL_REPORT.md"),
            "runtime": r34s.rel(STAGE / "runtime_full_thread8"),
        },
        "boundary": (
            "Post-R38 operation-surface repair. Success requires context-only-special action fidelity, "
            "beating high-risk reverse and temporal-random controls, plus default baseline gate."
        ),
    }
    r34s.write_json(STAGE / "summary/stage4_r39_lingbot_ta_midband_context_special_summary.json", summary)
    ...
    r34s.add_registry_row(
        {
            ...
            "status": decision,
            "artifact": r34s.rel(STAGE / "summary/stage4_r39_lingbot_ta_midband_context_special_summary.json"),
            "notes": "Post-R38 context-only-special repair using R37 midband de-clustered semantic-support carriers with high-risk reverse and temporal-random controls",
        }
    )
