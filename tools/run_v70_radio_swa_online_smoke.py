#!/usr/bin/env python3
"""Run ACL2 v70 RADIO SWA online local-window smoke jobs with a GPU queue."""
...
SWA_CASES: Dict[str, Dict[str, Any]] = {
    "native_no_swa": {
    ...
    "same_cue_random": {
        ...
        "mode": "source_random_same_mass",
    },
    "label_structure": {
        "enable_swa_overlap_source_gate": "0",
        "enable_swa_overlap_source_replace": "1",
        "read_cue_source": "dyn",
        "mode": "semantic_structure_highd_q90",
    },
}
...
def _swa_cases_for_cue(read_cue_source: str, mode: str, action: str) -> Dict[str, Dict[str, Any]]:
    cue = str(read_cue_source or DEFAULT_READ_CUE_SOURCE).strip()
    action = str(action or "replace").strip().lower()
    cases = {name: dict(cfg) for name, cfg in SWA_CASES.items()}
    for cfg in cases.values():
        cfg["enable_swa_overlap_source_gate"] = "1" if action == "gate" else "0"
        cfg["enable_swa_overlap_source_replace"] = "1" if action == "replace" else "0"
        cfg["turn_off_swa"] = "0"
    cases["native_no_swa"]["enable_swa_overlap_source_gate"] = "0"
    cases["native_no_swa"]["enable_swa_overlap_source_replace"] = "0"
    cases["candidate"]["read_cue_source"] = cue
    cases["candidate"]["mode"] = str(mode)
    if action == "leave_one_out":
        cases["candidate"]["enable_swa_overlap_source_gate"] = "0"
        cases["candidate"]["enable_swa_overlap_source_replace"] = "0"
        cases["candidate"]["turn_off_swa"] = "1"
        cases["candidate"]["component_intervention"] = "native_swa_leave_one_out"
    for case, suffix in {
    ...
    cases["same_cue_random"]["mode"] = f"{mode}_random_same_mass"
    return cases
...
def _build_command(args: argparse.Namespace, *, chunk: int, case: str, out_dir: Path) -> List[str]:
    mode = args.swa_gate_mode if str(args.swa_action).lower() == "gate" else args.swa_replace_mode
    case_cfg = _swa_cases_for_cue(args.read_cue_source, mode, args.swa_action)[case]
    ...
    cmd = [
        ...
        "--turn_off_swa",
        str(case_cfg.get("turn_off_swa", "0")),
        "--device",
        "cuda",
        "--hybrid_memory_mode",
        "read_path_only",
...
def main() -> None:
    ...
    parser.add_argument("--cases", default=",".join(SWA_CASES))
    ...
    parser.add_argument("--swa-replace-mode", default="source")
    parser.add_argument("--swa-replace-target", default="kv")
    parser.add_argument("--swa-replace-layer-mode", default="last")
    parser.add_argument("--swa-action", choices=("replace", "gate", "leave_one_out"), default="replace")
    parser.add_argument("--swa-gate-mode", default="intersection")
    parser.add_argument("--swa-gate-target", default="kv")
    parser.add_argument("--swa-gate-layer-mode", default="last")
    parser.add_argument("--swa-gate-rho", type=float, default=0.40)
    parser.add_argument("--swa-gate-min", type=float, default=0.85)
    ...
    parser.add_argument("--context-chunks", type=int, default=2,
                        help="number of consecutive chunks to run so SWA has previous source/history tokens")
    ...
    parser.add_argument("--swa-replace-alpha", type=float, default=0.40)
    ...
    mode = args.swa_gate_mode if str(args.swa_action).lower() == "gate" else args.swa_replace_mode
    swa_cases = _swa_cases_for_cue(args.read_cue_source, mode, args.swa_action)
    cases = [case.strip() for case in str(args.cases).split(",") if case.strip()]
    unknown = [case for case in cases if case not in swa_cases]
    if unknown:
        raise ValueError(f"unknown cases: {unknown}; known={sorted(swa_cases)}")
    ...
    for chunk in chunks:
        ...
        for case in cases:
            ...
            jobs.append({
                ...
                "read_cue_source_effective": swa_cases[case]["read_cue_source"],
                "swa_action_effective": str(args.swa_action),
                "swa_overlap_mode_effective": swa_cases[case]["mode"],
                "swa_overlap_source_replace_mode_effective": swa_cases[case]["mode"],
                "swa_overlap_source_gate_mode_effective": swa_cases[case]["mode"],
                "turn_off_swa_effective": swa_cases[case].get("turn_off_swa", "0"),
                "component_intervention_effective": swa_cases[case].get("component_intervention", ""),
    ...
    manifest_path = args.output_root / "v70_radio_swa_online_manifest.json"
