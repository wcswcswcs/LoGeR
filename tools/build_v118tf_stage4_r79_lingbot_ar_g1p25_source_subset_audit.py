#!/usr/bin/env python3
"""Build and summarize R79 g1p25 source-subset audit for seq08."""
...
STAGE = RESULT_ROOT / "stage4_r79_lingbot_ar_fresh_g1p25_source_subset_audit_08"
...
R78_KNOWN_S0124 = "lingbot_map_stream_sdpa_v118_r78_fresh_leaveout35_g1p25_seq08"
R77_BEST = "lingbot_map_stream_sdpa_v118_r77_fresh_exclude67_leaveout_3_seq08"
...
MIN_REL = 0.03
BASE_FRAMES = (0, 1, 2, 4, 5)
KNOWN_SUBSETS = {
    BASE_FRAMES: R78_BEST,
    (0, 1, 2, 4): R78_KNOWN_S0124,
}
...
def method_specs() -> list[dict[str, Any]]:
    ...
    for size in (4, 3):
        for source_frames in combinations(BASE_FRAMES, size):
            if source_frames in KNOWN_SUBSETS:
                continue
            frame_slug = "".join(str(frame) for frame in source_frames)
            role = f"g1p25_s{frame_slug}"
            method = f"lingbot_map_stream_sdpa_v118_r79_fresh_{role}_seq08"
            specs.append(
                {
                    "role": role,
                    "method": method,
                    "policy": f"R79_FRESH_{role.upper()}",
...
def method_yaml(policy: str, source_frames: tuple[int, ...], token_weight_mode: str) -> str:
...
def build_configs() -> list[dict[str, Any]]:
    ...
    config = CONFIG_DIR / "kitti_lingbot_sdpa_v118_r79_g1p25_source_subset_audit_seq08.yaml"
    write_text(config, main_config([str(spec["method"]) for spec in specs]))
    ...
    for idx, spec in enumerate(specs):
        ...
        write_text(CONFIG_DIR / "methods" / f"{method}.yaml", method_yaml(str(spec["policy"]), source_frames, token_weight_mode))
        ...
        method_rows.append(
            {
                "schema": "acl2_v118tf_stage4_r79_g1p25_source_subset_method_row_v1",
                "seq": SEQ,
                "dataset": DATASET,
                "role": spec["role"],
                "method": method,
        ...
        manifest_rows.append(
            {
                "schema": "acl2_v118tf_stage4_r79_g1p25_source_subset_run_manifest_row_v1",
                "phase": "cleanup_trace",
                "seq": SEQ,
                "dataset": DATASET,
                "method": method,
    ...
    manifest_rows.append(
        {
            ...
            "seq": SEQ,
            "dataset": DATASET,
            "method": ";".join(str(spec["method"]) for spec in specs),
            "gpu": "",
            "cwd": str(BENCH),
            "config": str(config.resolve()),
            "action_trace": "",
    ...
    write_csv(SUMMARY_DIR / "stage4_r79_g1p25_source_subset_method_rows.csv", method_rows)
...
def summarize(method_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    r78 = read_metric(R78_BEST)
    r78_known_s0124 = read_metric(R78_KNOWN_S0124)
    r77 = read_metric(R77_BEST)
    ...
    for spec in method_rows:
        ...
        row = {
            "schema": "acl2_v118tf_stage4_r79_g1p25_source_subset_result_row_v1",
            "seq": SEQ,
            "dataset": DATASET,
            "role": spec["role"],
            "method": method,
    ...
    decision = "R79_CONFIG_READY_NOT_RUN"
    if complete:
        if best.get("geometry_gate") and best.get("better_than_r78_best") and best.get("better_than_r71_original_random"):
            decision = "R79_G1P25_SOURCE_SUBSET_GEOMETRY_CANDIDATE_FOUND_REQUIRES_MATCHED_CONTROLS"
        elif best.get("better_than_r78_best"):
            decision = "R79_G1P25_SOURCE_SUBSET_IMPROVES_R78_BUT_GATE_OR_RANDOM_STILL_FAILS"
        elif best.get("geometry_gate"):
            decision = "R79_G1P25_SOURCE_SUBSET_GEOMETRY_GATE_WITHOUT_R78_IMPROVEMENT"
        else:
            decision = "R79_G1P25_SOURCE_SUBSET_NO_GEOMETRY_GATE_REPAIR"
    summary = {
        "schema": "acl2_v118tf_stage4_r79_g1p25_source_subset_summary_v1",
        "stage4_r79_decision": decision,
        "complete": complete,
        "global_goal_achieved": False,
        "claim_level": "fresh_seq08_g1p25_source_subset_search_only",
        "min_rel_gate": MIN_REL,
        "candidate_count": len(method_rows),
        "baseline": baseline,
        "r78_best": r78,
        "r78_known_s0124": r78_known_s0124,
        "r77_best": r77,
        "r74_exclude67": r74,
        "r71_original_random_control": prior_random,
        "known_subsets_reused_as_references": {
            ";".join(str(frame) for frame in subset): method for subset, method in KNOWN_SUBSETS.items()
        },
        "best_row": best,
        "result_rows": rows,
        "outputs": {
            "summary": rel(SUMMARY_DIR / "stage4_r79_g1p25_source_subset_summary.json"),
            "rows": rel(SUMMARY_DIR / "stage4_r79_g1p25_source_subset_rows.csv"),
            "method_rows": rel(SUMMARY_DIR / "stage4_r79_g1p25_source_subset_method_rows.csv"),
            "run_manifest": rel(STAGE / "run_manifest.csv"),
        },
    }
    write_csv(SUMMARY_DIR / "stage4_r79_g1p25_source_subset_rows.csv", rows)
    write_json(SUMMARY_DIR / "stage4_r79_g1p25_source_subset_summary.json", summary)
    return summary
