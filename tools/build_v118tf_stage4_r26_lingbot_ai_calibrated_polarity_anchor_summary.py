#!/usr/bin/env python3
"""Summarize ACL2 v118 Stage4-R26 calibrated-polarity LingBot anchor repair."""
...
MANIFEST = STAGE / "summary/stage4_r26_lingbot_ai_calibrated_polarity_anchor_manifest.json"
...
def summarize_rows(manifest: dict) -> list[dict]:
    ...
    for method, meta in manifest.get("methods", {}).items():
        for seq in ("00", "02"):
            ...
            rows.append(
                {
                    "schema": "acl2_v118tf_stage4_r26_lingbot_ai_calibrated_polarity_anchor_row_v1",
                    "dataset": DATASET,
                    "seq": seq,
                    "method": method,
                    "branch": "LB-AI",
...
def main() -> None:
    ...
    if not complete:
        decision = "AI_CALIBRATED_POLARITY_RUNTIME_INCOMPLETE"
    elif not action_fidelity:
        decision = "AI_CALIBRATED_POLARITY_ACTION_FIDELITY_NO_GO"
    elif candidate_better_all_controls and baseline_gate:
        decision = "AI_CALIBRATED_POLARITY_CANDIDATE_FOUND_REQUIRES_FRESH_HOLDOUT"
    else:
        decision = "AI_CALIBRATED_POLARITY_CONTROL_OR_BASELINE_NO_GO"

    out = STAGE / "summary"
    summary = {
        "schema": "acl2_v118tf_stage4_r26_lingbot_ai_calibrated_polarity_anchor_summary_v1",
        "stage4_r26_decision": decision,
        ...
        "boundary": (
            "R26 tests AI4 calibrated-polarity anchor initialization. The polarity rule uses only pre-run support/internal fields, "
            "but it was designed after R25 analysis, so a pass is treated as candidate-found requiring fresh holdout rather than global completion."
        ),
        "outputs": {
            "rows": str((out / "stage4_r26_lingbot_ai_calibrated_polarity_anchor_rows.csv").relative_to(ROOT)),
            "summary": str((out / "stage4_r26_lingbot_ai_calibrated_polarity_anchor_summary.json").relative_to(ROOT)),
            "report": str((out / "STAGE4_R26_LINGBOT_AI_CALIBRATED_POLARITY_ANCHOR_REPORT.md").relative_to(ROOT)),
        },
    }
    out.mkdir(parents=True, exist_ok=True)
    write_csv(out / "stage4_r26_lingbot_ai_calibrated_polarity_anchor_rows.csv", rows)
    (out / "stage4_r26_lingbot_ai_calibrated_polarity_anchor_summary.json").write_text(
        json.dumps(summary, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    report = [
        "# Stage4-R26 LingBot AI Calibrated Polarity Anchor Summary",
        "",
        f"decision: `{decision}`",
        ...
        "",
        "```json",
        json.dumps({"calibration_by_seq": summary["calibration_by_seq"], "comparisons": comparisons}, indent=2, sort_keys=True),
        "```",
    ]
    (out / "STAGE4_R26_LINGBOT_AI_CALIBRATED_POLARITY_ANCHOR_REPORT.md").write_text(
        "\n".join(report) + "\n",
        encoding="utf-8",
    )
