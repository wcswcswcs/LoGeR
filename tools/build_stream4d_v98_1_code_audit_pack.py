#!/usr/bin/env python3
"""Build a code-only Stream4D v98.1 review packet."""
...
NAME_PREFIX = "stream4d_v98_1_code_only_audit"


CODE_FILES = [
    "tools/build_stream4d_v98_1_code_audit_pack.py",
    ...
    "Stream3D/tools/run_v65_soma_pipeline_visualization.py",
...
README = """# Stream4D v98.1 code-only audit packet

This archive intentionally contains code only plus package verification metadata.

Included:
- v98.1 build/evaluation scripts
- v98.1 DA3/D4RT geometry-quality and dense-geometry visualization scripts
- MV_AP evaluator and v89 Stream3D local-window evaluator used by v98.1
...
- experiment logs / docs
- datasets, caches, checkpoints, model binaries, CSV outputs, masks, or prior audit packs

Review context:
Stream4D v98.1 geometry-contract, DA3/D4RT diagnostic visualization, MV_AP metric, and affinity-evaluation code.
"""
...
def write_metadata(staging: Path, copied: list[str]) -> None:
    ...
    (staging / "NO_ARTIFACTS_INCLUDED.txt").write_text(
        "This is a code-only packet. No files under Stream3D/outputs/audit are included.\n",
        encoding="utf-8",
    )
...
def validate_zip(zip_path: Path, expected_entries: list[str]) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="stream4d_v98_1_code_only_extract_") as tmp:
        tmp_path = Path(tmp)
        with zipfile.ZipFile(zip_path) as zf:
