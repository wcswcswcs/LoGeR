#!/usr/bin/env python3
"""Pairwise combo search for v100 head/layer semantic trace diagnostics."""
...
def main() -> None:
    ...
    rows = []
    for case_id in sorted(set(l2_rows) & set(head_rows)):
        extra = {
    ...
    specs = [
        ...
        ("headlayer_stable_anchor_topk_hit_frac_mean", "lower_bad"),
        ("headlayer_stable_anchor_topk_hit_frac_max", "lower_bad"),
        ("headlayer_stable_anchor_topk_hit_frac_top3_mean", "lower_bad"),
        ("headlayer_stable_anchor_topk_hit_frac_top3_mean", "higher_bad"),
        ("headlayer_head_low_fine_current_support_risk_mean", "higher_bad"),
        ("headlayer_head_low_fine_current_support_risk_mean", "lower_bad"),
        ("headlayer_head_low_fine_current_support_risk_top3_mean", "lower_bad"),
        ("headlayer_head_low_fine_current_support_risk_top5_mean", "lower_bad"),
        ("headlayer_head_low_group_current_support_risk_mean", "lower_bad"),
        ("headlayer_head_low_group_current_support_risk_top3_mean", "lower_bad"),
        ("headlayer_stable_anchor_same_fine_given_stable_frac_weighted", "lower_bad"),
        ("headlayer_low_fine_high_stable_head_layer_frac", "higher_bad"),
        ("headlayer_layer2_low_fine_risk_mean", "higher_bad"),
        ("headlayer_layer3_low_fine_risk_mean", "lower_bad"),
    ...
    summary = {
        ...
        "note": "Pairwise AND/OR diagnostic over L2 case rows plus head/layer semantic trace current-support rows.",
    }
    out = args.out_dir
    write_rows(out / "semantic_trace_head_layer_combo_search_top_metrics.csv", candidates[:500])
    write_json(out / "semantic_trace_head_layer_combo_search_summary.json", summary)
    report = [
        "# Semantic Trace Head/Layer Combo Search",
        "",
    ...
    (out / "semantic_trace_head_layer_combo_search_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")
