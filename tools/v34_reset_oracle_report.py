#!/usr/bin/env python3
"""Aggregate ACL2 v34 reset-group oracle short rollouts."""
...
def _status_done(run_dir: Path) -> bool:
    status = run_dir / "run_status.txt"
...
def main() -> int:
    ...
    parser.add_argument("--chunks", default="0,5,10,15,20,25,30")
    ...
    for chunk in chunks:
        ref_dir = rollout_root / _run_name(args.run_prefix, BASE_CANDIDATE, chunk, args.horizon)
        if not (ref_dir / "01.txt").exists() or not _status_done(ref_dir):
            missing.append({"chunk": chunk, "candidate": BASE_CANDIDATE, "reason": "missing_or_not_done_reference"})
            continue
        ...
        for candidate in candidates:
            ...
            if not (run_dir / "01.txt").exists() or not _status_done(run_dir):
                missing.append({"chunk": chunk, "candidate": candidate, "reason": "missing_or_not_done_prediction"})
                continue
            ...
            try:
                ref_subset = np.stack([h9_lookup[int(frame)] for frame in frames_eval], axis=0)
            except KeyError:
                missing.append({"chunk": chunk, "candidate": candidate, "reason": "reference_frame_mismatch"})
                continue
            aligned, metrics = _align_metrics(frames_eval, poses_eval, gt_poses, gt_pos)
            ref_aligned, ref_metrics = _align_metrics(frames_eval, ref_subset, gt_poses, gt_pos)
            ...
            ref_200 = _segment_ate(frames_eval, ref_aligned, gt_pos, 200, 300)
            seg_400 = _segment_ate(frames_eval, aligned, gt_pos, 400, 600)
            ref_400 = _segment_ate(frames_eval, ref_aligned, gt_pos, 400, 600)
            ate_delta = float(metrics["ATE_horizon"] - ref_metrics["ATE_horizon"])
            seg_200_delta = float(seg_200 - ref_200) if math.isfinite(seg_200) and math.isfinite(ref_200) else float("nan")
            seg_400_delta = float(seg_400 - ref_400) if math.isfinite(seg_400) and math.isfinite(ref_400) else float("nan")
            gate = (
                (
            ...
            rows.append(
                {
                    ...
                    "base_ATE_horizon": float(ref_metrics["ATE_horizon"]),
                    "ATE_delta_vs_base": ate_delta,
                    "intersection_200_300_ATE": seg_200,
                    "base_intersection_200_300_ATE": ref_200,
                    "intersection_200_300_delta_vs_base": seg_200_delta,
                    "intersection_400_600_ATE": seg_400,
                    "base_intersection_400_600_ATE": ref_400,
                    "intersection_400_600_delta_vs_base": seg_400_delta,
                    ...
                    "prior_v32_semantic_cue_active": debug.get("prior_v32_semantic_cue_active"),
                    "prior_v32_semantic_z_high_mass": debug.get("prior_v32_semantic_z_high_mass"),
                    "prior_v32_semantic_d_mean": debug.get("prior_v32_semantic_d_mean"),
                    "prior_v32_semantic_d_q90": debug.get("prior_v32_semantic_d_q90"),
                }
            )

    _write_csv(out_dir / f"{args.report_prefix}_effects.csv", rows)
    _write_json(out_dir / f"{args.report_prefix}_effects.json", rows)
    _write_csv(out_dir / f"{args.report_prefix}_missing_rows.csv", missing)
    ...
    for chunk in chunks:
        ...
        finite_seg = [r for r in subset if math.isfinite(float(r["intersection_200_300_delta_vs_base"]))]
        best_seg = min(finite_seg, key=lambda r: float(r["intersection_200_300_delta_vs_base"])) if finite_seg else None
        by_chunk.append(
            {
                "chunk": int(chunk),
                "rows": len(subset),
                ...
                "best_200_300_delta_vs_base": best_seg["intersection_200_300_delta_vs_base"] if best_seg else None,
                "best_400_600_delta_for_best_ATE": best_ate["intersection_400_600_delta_vs_base"],
                "gate_pass_candidates": [r["candidate"] for r in subset if r["gate_pass"]],
            }
        )
    _write_csv(out_dir / f"{args.report_prefix}_by_chunk.csv", by_chunk)

    positive_chunks = sorted({int(r["chunk"]) for r in rows if bool(r["gate_pass"])})
    non_chunk10_positive = [chunk for chunk in positive_chunks if chunk != 10]
    best_ate = min(rows, key=lambda r: float(r["ATE_delta_vs_base"]), default=None)
    finite_seg_rows = [r for r in rows if math.isfinite(float(r["intersection_200_300_delta_vs_base"]))]
    best_seg = min(finite_seg_rows, key=lambda r: float(r["intersection_200_300_delta_vs_base"]), default=None)
    summary = {
        "rows": len(rows),
        ...
        "non_chunk10_positive_chunks": non_chunk10_positive,
        "h1_multi_reset_positive": len(positive_chunks) >= 2 and bool(non_chunk10_positive),
        ...
        "best_200_300_delta_vs_base": best_seg["intersection_200_300_delta_vs_base"] if best_seg else None,
        "best_400_600_delta_for_best_ATE": best_ate["intersection_400_600_delta_vs_base"] if best_ate else None,
        "by_chunk": by_chunk,
    }
    _write_json(out_dir / f"{args.report_prefix}_summary.json", summary)
