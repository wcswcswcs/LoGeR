#!/usr/bin/env python3
"""Build ACL2 v118 Stage4-R26 calibrated-polarity LingBot anchor configs."""
...
STAGE = ROOT / "results/acl2_v118tf_operation_specific_semantic_carrier_calibration/stage4_r26_lingbot_ai_calibrated_polarity_anchor"
...
METHODS = {
    "lingbot_map_stream_flashinfer_v118_r26_ai4_calibrated_polarity_anchor_first32": {
        "policy": "AI4_CALIBRATED_POLARITY_ANCHOR_FIRST32",
        "role": "candidate",
        "description": "use high internal-QK only when high-QK top8 has stable-landmark support; otherwise flip to low internal-QK",
    },
    "lingbot_map_stream_flashinfer_v118_r26_ai4_opposite_polarity_anchor_first32": {
        "policy": "AI4_OPPOSITE_POLARITY_CONTROL_ANCHOR_FIRST32",
        "role": "opposite_polarity_control",
        "description": "use the opposite high/low internal-QK polarity for each sequence",
    },
    "lingbot_map_stream_flashinfer_v118_r26_ai4_random_polarity_pool_anchor_first32": {
        "policy": "AI4_MATCHED_RANDOM_POLARITY_POOL_ANCHOR_FIRST32",
...
def write_main_config(config_dir: Path) -> None:
    ...
    (config_dir / "kitti_lingbot_flashinfer_r26_ai_calibrated_polarity_anchor_full_reuse_v105gt.yaml").write_text(
        "\n".join(lines),
        encoding="utf-8",
    )
...
def write_manifest(rows: list[dict[str, Any]], seq_maps_by_method: dict[str, dict[str, list[int]]], calibration: dict[str, Any]) -> None:
    ...
    csv_path = out / "stage4_r26_lingbot_ai_calibrated_polarity_anchor_manifest.csv"
    ...
    payload = {
        "schema": "acl2_v118tf_stage4_r26_lingbot_ai_calibrated_polarity_anchor_manifest_v1",
    ...
    (out / "stage4_r26_lingbot_ai_calibrated_polarity_anchor_manifest.json").write_text(
...
def main() -> None:
    ...
    for seq in ("00", "02"):
        ...
        selections = {
            "lingbot_map_stream_flashinfer_v118_r26_ai4_calibrated_polarity_anchor_first32": candidate_frames,
            "lingbot_map_stream_flashinfer_v118_r26_ai4_opposite_polarity_anchor_first32": opposite_frames,
            "lingbot_map_stream_flashinfer_v118_r26_ai4_random_polarity_pool_anchor_first32": random_frames,
        }
        for method, frames in selections.items():
            seq_maps_by_method[method][seq] = frames
            manifest_rows.append(
                {
                    "schema": "acl2_v118tf_stage4_r26_lingbot_ai_calibrated_polarity_anchor_manifest_row_v1",
                    "seq": seq,
                    "method": method,
                    "role": METHODS[method]["role"],
                    "policy": METHODS[method]["policy"],
                    "calibrated_polarity": decision["polarity"] if METHODS[method]["role"] == "candidate" else (
                        "opposite_" + decision["polarity"] if METHODS[method]["role"] == "opposite_polarity_control" else "random"
                    ),
        ...
        json.dumps(
            {
                "stage": str(STAGE.relative_to(ROOT)),
                "config": str(
                    (config_dir / "kitti_lingbot_flashinfer_r26_ai_calibrated_polarity_anchor_full_reuse_v105gt.yaml").relative_to(ROOT)
                ),
